# AWS SAA-C03 Solutions Architect Associate - Exam Solutions Guide

> A comprehensive study guide with **634** questions, answers, and clear explanations.
...
| Range | Questions |
|-------|----------|
| [Q1 - Q50](#questions-1-to-50) | 46 questions |
| [Q51 - Q100](#questions-51-to-100) | 48 questions |
| [Q101 - Q150](#questions-101-to-150) | 50 questions |
| [Q151 - Q200](#questions-151-to-200) | 40 questions |
| [Q201 - Q250](#questions-201-to-250) | 44 questions |
| [Q251 - Q300](#questions-251-to-300) | 46 questions |
| [Q301 - Q350](#questions-301-to-350) | 45 questions |
| [Q351 - Q400](#questions-351-to-400) | 48 questions |
| [Q401 - Q450](#questions-401-to-450) | 44 questions |
| [Q451 - Q500](#questions-451-to-500) | 47 questions |
| [Q501 - Q550](#questions-501-to-550) | 47 questions |
| [Q551 - Q600](#questions-551-to-600) | 47 questions |
| [Q601 - Q650](#questions-601-to-650) | 48 questions |
| [Q651 - Q684](#questions-651-to-684) | 34 questions |
...
| Service | When to Use |
|---------|-------------|
| **S3** | Object storage, static websites, data lakes, backups |
| **S3 Glacier / Deep Archive** | Long-term archival, infrequent access, compliance |
| **S3 Intelligent-Tiering** | Unpredictable access patterns, auto cost optimization |
| **EC2** | Virtual servers, full OS/networking control |
| **Lambda** | Serverless functions, event-driven, <15 min tasks |
| **RDS** | Managed relational DB (MySQL, PostgreSQL, Oracle) |
| **Aurora** | High-perf MySQL/PostgreSQL, up to 5x faster |
| **Aurora Serverless** | Variable workloads, auto-scaling, pay-per-use DB |
| **DynamoDB** | NoSQL key-value, millisecond latency |
| **ElastiCache** | In-memory caching (Redis/Memcached) |
| **CloudFront** | CDN, global content delivery at edge locations |
...
| **Fargate** | Serverless containers, no infra management |
| **VPC Endpoint** | Private access to AWS services (no internet) |
| **NAT Gateway** | Internet for private subnets (outbound only) |
| **Transit Gateway** | Connect multiple VPCs and on-prem |
| **Direct Connect** | Dedicated private connection to AWS |
| **WAF** | Web firewall (SQL injection, XSS, rate limiting) |
| **Shield** | DDoS protection (Standard=free, Advanced=paid) |
| **Secrets Manager** | Store and auto-rotate credentials |
| **KMS** | Encryption key management |
| **CloudTrail** | API call logging and auditing |
| **AWS Config** | Resource config tracking and compliance |
| **DataSync** | Fast online data transfer to AWS |
| **Storage Gateway** | Hybrid storage (on-prem cache + S3) |
| **Snowball/Snowcone** | Offline bulk migration (10TB+) |
...
| **RDS Proxy** | Connection pooling (great with Lambda) |
| **Global Accelerator** | Improve global perf via AWS backbone |
| **AWS Backup** | Centralized backup management |
| **EventBridge** | Event routing, scheduled tasks |

---

<a id="questions-1-to-50"></a>
...
**Correct Answer:** Use Amazon Athena directly with Amazon S3 to run the queries as needed.
...
**Simple Explanation:**
> Option B: Amazon S3 File Gateway provides a hybrid cloud storage solution, integrating on-premises environments with cloud storage. Files written to the file share are automatically saved as S3 objects. With S3 Lifecycle policies, you can transition objects between storage classes.
...
**Correct Answer:** A. Use AWS Secrets Manager and attach an IAM role that grants access to that secret to the EC2 instances that need it. Turn on automatic rotation.
...
**Correct Answer:** B. Create an analysis in Amazon QuickSight. Connect all the data sources and create new datasets. Publish dashboards to visualize the data. Share the dashboards with the appropriate users and groups
...
**Correct Answer:** A. Create an IAM role that grants access to the S3 bucket. Attach the role to the EC2 instances
...
**Correct Answer:** A. Turn on AWS Config with the appropriate rules.
...
**Correct Answer:** A. Share the dashboard from the CloudWatch console. Enter the product manager's email address, and complete the sharing steps. Provide a shareable link for the dashboard to the product manage.
...
**Correct Answer:** A. Enable AWS Single Sign-On (AWS SSO) from the AWS SSO console. Create a one-way forest trust or a one-way domain trust to connect the company's self-managed Microsoft Active Directory with AWS SSO by using AWS Directory Service for Microsoft Active Directory.
...
**Simple Explanation:**
> Answer A does not mention service Global Accelerator, it mentions the NLB would act like one. Not sure if the wording is wrong or not. ' Deploy a Network Load Balancer (NLB) and an associated target group.
...
**Correct Answer:** A. Use AWS Config rules to define and detect resources that are not properly tagged.
...
**Correct Answer:** B. Attach the appropriate IAM role to each existing instance and new instance. Use AWS Systems Manager Session Manager to establish a remote SSH session.
...
**Correct Answer:** A. Create an Amazon Kinesis Data Firehose delivery stream to ingest the alerts. Configure the Kinesis Data Firehose stream to deliver the alerts to an Amazon S3 bucket. Set up an S3 Lifecycle configuration to transition data to Amazon S3 Glacier after 14 days.

**Simple Explanation:**
> Amazon Kinesis Data Firehose is a fully managed service that can capture, transform, and deliver streaming data into storage systems or analytics tools, making it an ideal solution for ingesting and storing status alerts. In this solution, the Kinesis Data Firehose delivery stream ingests the alerts and delivers them to an S3 bucket, which is a cost-effective storage solution.
...
**Correct Answer:** B. Create an Amazon AppFlow flow to transfer data between each SaaS source and the S3 bucket. Configure an S3 event notification to send events to an Amazon Simple Notification Service (Amazon SNS) topic when the upload to the S3 bucket is complete.

**Simple Explanation:**
> Amazon AppFlow is a fully-managed integration service that enables you to securely exchange data between software as a service (SaaS) applications, such as Salesforce, and AWS services, such as Amazon Simple Storage Service (Amazon S3) and Amazon Redshift. The use of Appflow helps to remove the ec2 as the middle layer which slows down the process of data transmission and introduce an additional variable.
...
**Correct Answer:** A. Enable versioning on the S3 bucket

**Simple Explanation:**
> The two most effective steps a solutions architect can take to protect an Amazon S3 bucket from accidental deletion are: Versioning keeps multiple versions of objects in the S3 bucket, even when they are overwritten or deleted. This allows you to recover objects that have been accidentally deleted. MFA Delete requires you to enter a one-time password from a multi-factor authentication (MFA) device before you can delete an object in the S3 bucket.
...
**Correct Answer:** B. Create an Amazon Simple Queue Service (Amazon SQS) queue, and subscribe it to the SNS topic.
...
**Correct Answer:** B. Use an Amazon S3 bucket as a secure transfer point. Use Amazon Macie to scan the objects in the bucket. If objects contain PII, use Amazon Simple Notification Service (Amazon SNS) to trigger a notification to the administrators to remove the objects that contain PII.

**Simple Explanation:**
> Some quotas can be increased, while others cannot. To request an increase to a quota, use the Service Quotas console. To learn how to request an increase, see Requesting a quota increase in the Service Quotas User Guide.
...
**Correct Answer:** D. Create an On-Demand Capacity Reservation that specifies the Region and three Availability Zones needed.
...
**Correct Answer:** D. Move the catalog to an Amazon Elastic File System (Amazon EFS) file system.
...
**Correct Answer:** C. Migrate the application to Amazon EC2 instances in a Multi-AZ Auto Scaling group. Use Amazon Elastic File System (Amazon EFS) for storage.
...
**Correct Answer:** C. Use an S3 Lifecycle policy to transition the records from S3 Standard to S3 Glacier Deep Archive after 1 year. Use S3 Object Lock in compliance mode for a period of 10 years.
...
**Correct Answer:** C. Extend the file share environment to Amazon FSx for Windows File Server with a Multi-AZ configuration. Migrate all the data to FSx for Windows File Server.
...
**Correct Answer:** C. Create a security group that allows inbound traffic from the security group that is assigned to instances in the private subnets. Attach the security group to the DB instances.
...
**Correct Answer:** C. Create a Regional API Gateway endpoint. Associate the API Gateway endpoint with the company's domain name. Import the public
...
**Correct Answer:** B. Use Amazon Rekognition to detect inappropriate content. Use human review for low-confidence predictions.
...
**Correct Answer:** D. Collect the data from Amazon Kinesis Data Streams. Use Amazon Kinesis Data Firehose to transmit the data to an Amazon S3 data lake. Load the data in Amazon Redshift for analysis.
...
**Correct Answer:** D. Deploy and configure Amazon FSx for Windows File Server on AWS. Deploy and configure an Amazon FSx File Gateway on premises. Move the on-premises file data to the FSx File Gateway. Configure the cloud workloads to use FSx for Windows File Server on AWS. Configure the on-premises workloads to use the FSx File Gateway.
...
**Correct Answer:** C. Create an S3 bucket lifecycle policy to move files from S3 Standard to S3 Standard-Infrequent Access (S3 Standard-IA) 30 days from object creation. Delete the files 4 years after object creation.
...
**Correct Answer:** A. Provision an AWS Direct Connect connection to a Region. Provision a VPN connection as a backup if the primary Direct Connect connection fails.
...
**Correct Answer:** C. Replace the NLB with an Application Load Balancer. Enable HTTP health checks by supplying the URL of the company's application. Configure an Auto Scaling action to replace unhealthy instances.
...
**Correct Answer:** C. Replace the current security group of the bastion host with one that only allows inbound access from the external IP range for the company.
...
**Correct Answer:** A. Configure the security group for the web tier to allow inbound traffic on port 443 from 0.0.0.0/0.
...
**Correct Answer:** C. Configure an Amazon API Gateway API to send data to an Amazon Kinesis data stream. Create an Amazon Kinesis Data Firehose delivery stream that uses the Kinesis data stream as a data source. Use AWS Lambda functions to transform the data. Use the Kinesis Data Firehose delivery stream to send the data to Amazon S3.
...
**Correct Answer:** C. Create an Amazon SQS queue to hold the jobs that need to be processed. Create an Amazon Machine Image (AMI) that consists of the processor application. Create a launch template that uses the AMI. Create an Auto Scaling group using the launch template. Set the scaling policy for the Auto Scaling group to add and remove nodes based on the number of items in the SQS queue.
...
**Correct Answer:** D. Create an Amazon EventBridge (Amazon CloudWatch Events) rule to detect any certificates that will expire within 30 days. Configure the rule to invoke an AWS Lambda function. Configure the Lambda function to send a custom alert by way of Amazon Simple Notification Service (Amazon SNS).
...
**Correct Answer:** A. Store the uploaded documents in an Amazon S3 bucket with S3 Versioning and S3 Object Lock enabled.
...
**Correct Answer:** A. Store the database user credentials in AWS Secrets Manager. Grant the necessary IAM permissions to allow the web servers to access AWS Secrets Manager.
...
**Correct Answer:** A. Configure the Requester Pays feature on the company's S3 bucket.
...
**Correct Answer:** B. Create a read replica of the database. Configure the script to query only the read replica.
...
**Correct Answer:** A. Configure a VPC gateway endpoint for Amazon S3 within the VPC.
...
**Correct Answer:** B. Use Amazon Aurora MySQL with Multi-AZ Aurora Replicas for production. Use database cloning to create the staging database on-demand.
...
**Correct Answer:** C. Configure Amazon S3 to send an event notification to an Amazon Simple Queue Service (Amazon SQS) queue. Use an AWS Lambda function to read from the queue and process the data. Store the resulting JSON file in Amazon DynamoDB.
...
**Correct Answer:** D. Create an Amazon FSx for Windows File Server file system on AWS and set the Active Directory domain for authentication.

---

### Q98. **[IMPORTANT]** `Storage`
...
**Correct Answer:** C. Increase the visibility timeout in the SQS queue to a value that is greater than the total of the function timeout and the batch window
...
**Correct Answer:** C. Create an AWS Key Management Service (AWS KMS) customer managed key. Allow the EC2 role to use the KMS key for encryption
...
**Correct Answer:** A. Launch the EC2 instance into the same Availability Zone as the EFS file system.
...
**Correct Answer:** A. Use AWS Shield Advanced to stop the DDoS attack.
...
**Correct Answer:** D. Add a resource-based policy to the function with lambda:InvokeFunction as the action and Service: events.amazonaws.com as the principal.
...
**Correct Answer:** D. Use Amazon API Gateway with Amazon Kinesis Data Analytics.
...
**Correct Answer:** A. Create an AWS Lambda function triggered when the database on Amazon RDS is updated to send the information to an Amazon Simple
...
**Correct Answer:** B. Configure the web server to upload the original images to Amazon S3.
...
**Correct Answer:** A. Use AWS Fargate on Amazon Elastic Container Service (Amazon ECS) to run the containerized web application with Service Auto Scaling. Use an Application Load Balancer to distribute the incoming requests.
...
**Correct Answer:** C. Use AWS Lambda to process the photos. Store the photos in Amazon S3. Retain DynamoDB to store the metadata.
...
**Correct Answer:** C. Move the EC2 instances to private subnets. Create a VPC endpoint for Amazon S3, and link the endpoint to the route table for the private subnets.
...
**Correct Answer:** A. Configure Amazon CloudFront in front of the website to use HTTPS functionality.
...
**Correct Answer:** C. Create an Amazon Kinesis Data Firehose delivery stream. Configure the log group as the delivery streams sources. Configure Amazon OpenSearch Service (Amazon Elasticsearch Service) as the delivery stream's destination.
...
**Correct Answer:** A. Set up AWS WAF in both Regions. Associate Regional web ACLs with an API stage.
...
**Correct Answer:** A. Create an Amazon Route 53 geolocation routing policy to route requests to one of the two NLBs. Create an Amazon CloudFront distribution. Use the Route 53 record as the distribution’s origin.
...
**Correct Answer:** A. Encrypt a copy of the latest DB snapshot. Replace existing DB instance by restoring the encrypted snapshot.
...
**Correct Answer:** D. Import the SSL certificate into AWS Certificate Manager (ACM). Create an Application Load Balancer with an HTTPS listener that uses the SSL certificate from ACM.

---

### Q124. **[IMPORTANT]** `Compute`
...
**Correct Answer:** A. Use an Auto Scaling group to launch the EC2 instances in private subnets. Deploy an RDS Multi-AZ DB instance in private subnets.
...
**Correct Answer:** B. Set up an S3 Lifecycle policy to transition objects to S3 Glacier Deep Archive after 2 years.
...
**Correct Answer:** A. Amazon EBS for maximum performance, Amazon S3 for durable data storage, and Amazon S3 Glacier for archival storage
...
**Correct Answer:** A. Use Spot Instances in an Amazon EC2 Auto Scaling group to run the application containers.
...
**Correct Answer:** A. Migrate the PostgreSQL database to Amazon Aurora.
...
**Correct Answer:** A. Create a new S3 bucket. Load the data into the new S3 bucket. Use S3 Cross-Region Replication (CRR) to replicate encrypted objects to an S3 bucket in another Region. Use server-side encryption with AWS KMS multi-Region kays (SSE-KMS). Use Amazon Athena to query the data.
...
**Correct Answer:** D. Ask the provider to create a VPC endpoint for the target service. Use AWS PrivateLink to connect to the target service.

---

### Q136. **[IMPORTANT]** `Database`
...
**Correct Answer:** A. Create an ongoing replication task.
...
**Correct Answer:** D. Configure all existing AWS accounts and all newly created accounts to use the same root user email address. Configure AWS account alternate contacts in the AWS Organizations console or programmatically.
...
**Correct Answer:** A. Create a Lambda function to copy the files to the analysis S3 bucket. Create an S3 event notification for the analysis S3 bucket. Configure Lambda and SageMaker Pipelines as destinations of the event notification. Configure s3:ObjectCreated:Put as the event type.

---

### Q140. **[IMPORTANT]** `Compute`
...
**Correct Answer:** A. Use Spot Instances for the data ingestion layer
...
**Correct Answer:** B. Deploy the application stack in two AWS Regions. Use an Amazon Route 53 latency routing policy to serve all content from the ALB in the closest Region.
...
**Correct Answer:** C. Configure AWS Global Accelerator to forward requests to a Network Load Balancer. Use Amazon EC2 instances for the application in an EC2 Auto Scaling group.
...
**Correct Answer:** D. Migrate the database to an Amazon Aurora MySQL DB instance. Create an AMI of the web application. Apply the AMI to a launch template. Create an Auto Scaling group with the launch template Configure the launch template to use a Spot Fleet. Attach an Application Load Balancer to the Auto Scaling group.
...
**Correct Answer:** B. Use Reserved Instances for the baseline level of usage. Use Spot instances for any additional capacity that the application needs.
...
**Correct Answer:** B. Store the logs in Amazon S3. Use S3 Lifecycle policies to move logs more than 1 month old to S3 Glacier Deep Archive.

---

### Q148. **[IMPORTANT]** `Compute`
...
<a id="questions-151-to-200"></a>

## Questions 151 to 200

### Q151. **[IMPORTANT]** `Networking`
...
**Correct Answer:** A. Use AWS Control Tower to implement data residency guardrails to deny internet access and deny access to all AWS Regions except ap-northeast-3.
...
**Correct Answer:** D. Create AWS Lambda functions to start and stop the DB instance. Create Amazon EventBridge (Amazon CloudWatch Events) scheduled rules to invoke the Lambda functions. Configure the Lambda functions as event targets for the rules.
...
**Correct Answer:** D. Implement an S3 Lifecycle policy that moves the objects from S3 Standard to S3 Standard-Infrequent Access (S3 Standard-1A) after 90 days.
...
**Correct Answer:** C. Deploy Amazon CloudFront to connect the S3 buckets to CloudFront edge servers.
...
**Correct Answer:** A. Use Amazon Athena for one-time queries. Use Amazon QuickSight to create dashboards for KPIs.

---

### Q157. **[IMPORTANT]** `Database`
...
**Correct Answer:** A. Take a manual snapshot of the DB cluster.

---

### Q158. **[IMPORTANT]** `Networking`
...
**Correct Answer:** A. Create a usage plan with an API key that is shared with genuine users only.

---

### Q160. **[IMPORTANT]** `General`
...
**Correct Answer:** A. Amazon OpenSearch Service (Amazon Elasticsearch Service)
...
**Correct Answer:** B. Place the JSON documents in an Amazon S3 bucket. Create an AWS Lambda function that runs the Python code to process the documents as they arrive in the S3 bucket. Store the results in an Amazon Aurora DB cluster.
...
**Correct Answer:** C. Use Reserved Instances for the baseline capacity and use Spot Instances to handle additional capacity.
...
**Correct Answer:** C. Configure a CloudFront field-level encryption profile.

**Simple Explanation:**
> Field-level encryption adds an extra layer of security on top of HTTPS. It encrypts specific sensitive fields (like credit card numbers) at the edge using a public key you provide. The data stays encrypted through the entire application stack — only the application with the corresponding private key can decrypt those fields. Signed URLs/cookies control *who can access* content but do not encrypt individual fields end-to-end. HTTPS Only protects data in transit but not at the application layer.
...
**Correct Answer:** B. Deploy an Amazon CloudFront web distribution in front of the S3 bucket.
...
**Correct Answer:** B. Modify the Auto Scaling group to use three instances across each of two Availability Zones.
...
**Correct Answer:** B. Use Amazon RDS Proxy to create a proxy for the database. Modify the Lambda function to use the RDS Proxy endpoint instead of the database endpoint.
...
**Correct Answer:** B. Use Amazon DynamoDB Accelerator (DAX).

---

### Q178. **[IMPORTANT]** `Compute`
...
**Correct Answer:** B. Use AWS Shield Advanced with the NLB. + C. Use AWS WAF to protect Amazon API Gateway.

**Simple Explanation:**
> This is a "Choose two" question. Shield Advanced provides DDoS protection for the NLB (layer 3/4 attacks). AWS WAF on API Gateway protects against web exploits like SQL injection and XSS (layer 7 attacks). WAF cannot be attached directly to an NLB — it only works with ALB, CloudFront, API Gateway, and AppSync. So the combination of Shield Advanced (DDoS) + WAF on API Gateway (web exploits) provides the most complete protection.
...
**Correct Answer:** A. Create an Amazon Simple Queue Service (Amazon SQS) queue. Add code to the data producers, and send data to the queue. Add code to the data consumers to process data from the queue.
...
**Correct Answer:** B. Create an Amazon RDS MySQL DB instance with Multi-AZ functionality enabled to synchronously replicate the data.
...
**Correct Answer:** A. Configure the Lambda function to run in the VPC with the appropriate security group.
...
**Simple Explanation:**
> Amazon FSx for Windows File Server provides a fully managed Windows file system that can be accessed by multiple EC2 instances. It is designed to provide shared file storage for Windows-based applications. It supports the SMB protocol, allowing Windows instances to mount the file system.
...
**Correct Answer:** A. Create an Amazon RDS DB instance in Multi-AZ mode.
...
**Correct Answer:** A. Use AWS Transfer Family to configure an SFTP-enabled server with a publicly accessible endpoint. Choose the S3 data lake as the destination.
...
**Correct Answer:** B. Store the documents in Amazon S3. Use S3 Object Lock in compliance mode. + D. Use SSE-KMS with a customer managed AWS KMS key. Enable automatic key rotation.

**Simple Explanation:**
> This is a "Choose two" question. S3 Object Lock in compliance mode prevents documents from being deleted or overwritten for the 5-year retention period (WORM model — even the root user cannot override). SSE-KMS with a customer managed key provides encryption at rest, and enabling automatic key rotation on the KMS key rotates the encryption key every year automatically. Together, these two choices meet all requirements: immutability (Object Lock) + encryption + auto key rotation (KMS).
...
**Correct Answer:** D. Add an Auto Scaling group for the application that sends meeting invitations. Configure the Auto Scaling group to scale based on the depth of the SQS queue.
...
**Correct Answer:** C. Create a private Amazon S3 bucket. Use an S3 bucket policy to allow access from a CloudFront origin access identity (OAI). Upload website content by using the AWS CLI.
...
**Correct Answer:** C. Create an Amazon EventBridge (Amazon CloudWatch Events) rule for the CreateImage API call. Configure the target as an Amazon Simple Notification Service (Amazon SNS) topic to send an alert when a CreateImage API call is detected.
...
**Correct Answer:** A. Create an interface VPC endpoint for Amazon S3 in the subnet where the EC2 instance is located. Attach a resource policy to the S3 bucket to only allow the EC2 instance’s IAM role for access.
...
**Simple Explanation:**
> Amazon ElastiCache is a fully managed, in-memory data store service. It is commonly used for caching and session management in distributed applications. By utilizing ElastiCache for session data management, you can store and retrieve session data in a scalable and high-performance manner.
...
**Correct Answer:** B. Create an AWS Glue crawler to discover the data. Create an AWS Glue extract, transform, and load (ETL) job to transform the data. Specify the transformed data bucket in the output step.

**Simple Explanation:**
> AWS Glue is a fully managed extract, transform, and load (ETL) service that makes it easy to prepare and load data for analysis. In this scenario: The AWS Glue crawler can automatically discover the schema of your data stored in Amazon S3, including the .csv files. The AWS Glue ETL job allows you to define the transformation logic easily.
...
**Correct Answer:** A. Order AWS Snowball devices to transfer the data. Use a lifecycle policy to transition the files to Amazon S3 Glacier Deep Archive.
...
**Correct Answer:** B. Turn on the default encryption settings for the S3 bucket. Use the S3 Inventory feature to create a .csv file that lists the unencrypted objects. Run an S3 Batch Operations job that uses the copy command to encrypt those objects.
...
**Correct Answer:** A. Create a security group with a rule to allow TCP port 443 from source 0.0.0.0/0.
...
**Correct Answer:** A. Create an IAM role in the company’s account to delegate access to the vendor’s IAM role. Attach the appropriate IAM policies to the role for the permissions that the vendor requires.

**Simple Explanation:**
> IAM roles allow you to delegate access to resources in your AWS account to another AWS account. In this case, you can create a role in your account and grant the vendor's IAM role permission to assume that role. By doing this, the vendor can use temporary security credentials obtained by assuming the role to access resources in your account.
...
**Simple Explanation:**
> Amazon Kinesis Data Firehose: It is a fully managed service that simplifies the delivery of streaming data to destinations such as Amazon S3, Amazon Redshift, or Amazon Elasticsearch Service. It handles the scaling, buffering, and delivery of data. Amazon Redshift: It is a fully managed, petabyte-scale data warehouse service.
...
**Correct Answer:** A. Use AWS Glue to process the raw data in Amazon S3.
...
**Correct Answer:** B. Configure the S3 Lifecycle policy to delete previous versions as well as current versions.
...
**Correct Answer:** C. Modify the API to write incoming data to an Amazon Simple Queue Service (Amazon SQS) queue. Use an AWS Lambda function that Amazon SQS invokes to write data from the queue to the database.
...
A company runs demonstration environments for its customers on Amazon EC2 instances. Each environment is isolated in its own VPC. The company’s operations team needs to be notified when RDP or SSH access to an environment has been established. B. Configure the EC2 instances with an IAM instance profile that has an IAM role with the AmazonSSMManagedInstanceCore policy attached. The correct option for notifying the operations team when RDP or SSH access to an environment has been established is:

**Correct Answer:** C. Publish VPC flow logs to Amazon CloudWatch Logs. Create required metric filters. Create an Amazon CloudWatch metric alarm with a notification action for when the alarm is in the ALARM state.

**Simple Explanation:**
> Here's why: VPC Flow Logs:** VPC flow logs capture information about the IP traffic going to and from network interfaces in your VPC. This includes information about accepted and rejected connections. To monitor RDP or SSH access, you can analyze these logs.
...
**Correct Answer:** A. Ensure the root user uses a strong password.

**Simple Explanation:**
> Using a strong, complex password for the root user is a fundamental security practice. This helps protect the account from unauthorized access. Enabling MFA adds an additional layer of security.
...
**Correct Answer:** C. Use AWS Key Management Service (AWS KMS) to encrypt the EBS volumes and Aurora database storage at rest. Attach an AWS Certificate Manager (ACM) certificate to the ALB to encrypt data in transit.

**Simple Explanation:**
> Using AWS KMS to encrypt EBS volumes and Aurora database storage at rest is a good practice. You can specify a KMS key when creating these resources to ensure data encryption. Attaching an ACM certificate to the ALB allows you to use HTTPS, which encrypts data in transit between clients and the ALB.
...
**Correct Answer:** D. Use load-balanced Multi-AZ AWS Elastic Beanstalk environments for the front-end layer and the application layer. Move the database to an Amazon RDS Multi-AZ DB instance. Use Amazon S3 to store and serve users’ images.

**Simple Explanation:**
> AWS Elastic Beanstalk provides an easy way to deploy and manage applications. By using Multi-AZ environments, the front-end and application layers can automatically scale and provide high availability across multiple Availability Zones (AZs). Amazon RDS Multi-AZ DB Instance: Moving the database to an Amazon RDS Multi-AZ DB instance ensures high availability and automatic failover in the event of a failure in one Availability Zone.
...
**Correct Answer:** C. Use AWS Budgets to create a cost budget for each account. Set the period to monthly. Set the scope to EC2 instances. Set an alert threshold for the budget. Configure an Amazon Simple Notification Service (Amazon SNS) topic to receive a notification when a threshold is exceeded.
...
**Correct Answer:** D. Host the visualization tool in the same AWS Region as the data warehouse and access it over a Direct Connect connection at a location in the same Region.
...
**Correct Answer:** C. Multivalue routing policy

**Simple Explanation:**
> The multivalue routing policy returns multiple healthy IP addresses for the resource in response to DNS queries. This is suitable for distributing traffic across multiple resources, such as EC2 instances, and meeting the specified requirement. Simple Routing: Gives one answer (IP address).
...
**Correct Answer:** A. Deploy an AWS Storage Gateway file gateway as a virtual machine (VM) on premises at each clinic
...
**Correct Answer:** C. Move the database to Amazon Aurora with a read replica in another Availability Zone. Create an Amazon Machine Image (AMI) from the EC2 instance. Configure an Application Load Balancer in two Availability Zones. Attach an Auto Scaling group that uses the AMI across two Availability Zones.
...
**Correct Answer:** D. Create public subnets in each Availability Zone. Associate the public subnets with the ALB. Update the route tables for the public subnets with a route to the private subnets.
...
**Correct Answer:** D. Route incoming requests to Amazon Simple Queue Service (Amazon SQS). Configure an EC2 Auto Scaling group based on queue size. Update the software to read from the queue.
...
249Topic 1 A company is implementing a shared storage solution for a media application that is hosted in the AWS Cloud. The company needs the ability to use SMB clients to access data. The solution must be fully managed. Which AWS solution meets these requirements?

**Correct Answer:** D. Create an Amazon FSx for Windows File Server file system. Attach the file system to the origin server. Connect the application server to the file system.
...
**Correct Answer:** D. Use Amazon S3 as the target. Enable an S3 Lifecycle policy to transition the logs to S3 Standard-Infrequent Access (S3 Standard-IA) after 90 days.
...
<a id="questions-251-to-300"></a>

## Questions 251 to 300
...
**Correct Answer:** B. Create a NAT gateway, and place it in a public subnet. Configure the private subnet route table to use the NAT gateway as the default route.
...
**Correct Answer:** D. Store the order in the database. Send a message that includes the order number to an Amazon Simple Queue Service (Amazon SQS) FIFO queue. Set the payment service to retrieve the message and process the order. Delete the message from the queue.
...
**Correct Answer:** B. Enable versioning on the bucket.
...
**Correct Answer:** D. Create an AWS Glue extract, transform, and load (ETL) job to convert the .csv files to Parquet format and place the output files into an S3 bucket. Create an AWS Lambda function for each S3 PUT event to invoke the ETL job.

**Simple Explanation:**
> AWS Glue ETL Job: AWS Glue is a fully managed extract, transform, and load (ETL) service that can be used to convert data formats. By creating an AWS Glue ETL job, you can offload the conversion process to a fully managed service, reducing operational overhead. AWS Lambda for S3 PUT Events: AWS Lambda can be configured to trigger on S3 PUT events.
...
**Correct Answer:** A. Create a backup vault in AWS Backup to retain RDS backups. Create a new backup plan with a daily schedule and an expiration period of 2 years after creation. Assign the RDS DB instances to the backup plan.
...
**Correct Answer:** D. Join the file system to the Active Directory to restrict access.
...
**Correct Answer:** A. Configure Amazon CloudFront to cache multiple versions of the content.

**Simple Explanation:**
> Amazon CloudFront is a content delivery network (CDN) service that can cache and deliver content globally. Configure CloudFront to cache different versions of content based on the device type or other criteria. Lambda@Edge allows you to run code in response to CloudFront events globally.
...
**Correct Answer:** A. Create a peering connection between the VPCs. Add a route table entry for the peering connection in both VPCs. Configure an inbound rule for the ElastiCache cluster’s security group to allow inbound connection from the application’s security groups.
...
**Correct Answer:** A. Deploy an Amazon Elastic Container Service (Amazon ECS) cluster.

**Simple Explanation:**
> An ECS cluster is necessary to organize and manage your Fargate tasks and services. It provides a logical grouping of tasks and services. When using Fargate, you don't need to manage the underlying EC2 instances; the cluster helps manage the Fargate tasks.
...
**Correct Answer:** D. Create an Application Load Balancer (ALB) with a health check in front of the EC2 instances. Route to the ALB from Route 53.
...
**Correct Answer:** C. Configure a public Application Load Balancer (ALB) with multiple redundant Amazon EC2 instances in private subnets. Configure Amazon CloudFront to deliver HTTPS content using the public ALB as the origin.

**Simple Explanation:**
> Public ALB in Private Subnets: Deploy a public Application Load Balancer (ALB) in private subnets. This ensures that the ALB is not directly accessible from the internet, providing an additional layer of security. Deploy multiple redundant Amazon EC2 instances in private subnets behind the ALB.
...
**Correct Answer:** A. Configure an accelerator in AWS Global Accelerator. Add a listener for the port that the application listens on, and attach it to a Regional endpoint in each Region. Add the ALB as the endpoint.
...
**Correct Answer:** D. Create an Amazon Kinesis Data Firehose delivery stream to store the data in Amazon S3. Create an Amazon Kinesis Data Analytics application to analyze the data.
...
**Correct Answer:** B. Use RDS Proxy between the application and the database.
...
**Correct Answer:** C. Configure scheduled scaling to scale up to the desired compute level.
...
**Correct Answer:** B. Configure an Amazon CloudFront distribution with the ALB as the origin. Set the cache behavior settings to cache based on the Accept-Language request header.
...
**Correct Answer:** B. Use an Amazon Aurora global database with a warm standby deployment.
...
**Correct Answer:** B. Create Amazon Machine Images (AMIs) to back up the EC2 instances. Copy the AMIs to a secondary AWS Region. Automate infrastructure deployment in the secondary Region by using AWS CloudFormation.
...
**Correct Answer:** C. Implement a target tracking action triggered at a lower CPU threshold, and decrease the cooldown period.
...
**Correct Answer:** D. Use Amazon S3 for storing the video content. Move the files temporarily over to an Amazon Elastic Block Store (Amazon EBS) volume attached to the server for processing.
...
**Correct Answer:** B. Use Amazon DynamoDB to store the employee data in hierarchies. Export the data to Amazon S3 every month.
...
**Correct Answer:** B. Use standard SQL queries in Amazon Athena to analyze the CloudFront logs in the S3 bucket. Visualize the results with Amazon QuickSight.
...
**Correct Answer:** B. Create an Amazon API Gateway endpoint with an AWS Lambda backend that makes a call to Amazon Simple Email Service (Amazon SES).

**Simple Explanation:**
> Amazon API Gateway: Acts as the endpoint for the contact form. It enables you to create, publish, maintain, monitor, and secure APIs at any scale. AWS Lambda: Serves as the backend for handling the dynamic components of the contact form.
...
**Correct Answer:** C. Invalidate the CloudFront cache.

**Simple Explanation:**
> When the website does not reflect updates that have been made in the Git repository, and the CI/CD pipeline is sending messages indicating successful deployments, it's likely that the issue is related to caching. Amazon CloudFront caches content to improve performance and reduce latency, and if the cache is not updated, it may serve stale content. By invalidating the CloudFront cache, you ensure that the next request to CloudFront fetches the latest content from the origin (in this case, Amazon S3).
...
**Correct Answer:** C. Create an Amazon Elastic File System (Amazon EFS) file system. Mount the EFS file system on all web servers.
...
**Simple Explanation:**
> An IAM role provides temporary credentials to the Lambda function to access AWS resources. The function does not have persistent credentials. The IAM policy grants least privilege access by specifying read access only to the specific S3 bucket needed.
...
**Correct Answer:** A. Signed cookies
...
**Correct Answer:** A. Use Amazon Kinesis Data Streams to stream the data. Use Amazon Kinesis Data Analytics to transform the data. Use Amazon Kinesis Data Firehose to write the data to Amazon S3. Use Amazon Athena to query the transformed data from Amazon S3.

**Simple Explanation:**
> Use Amazon Kinesis Data Streams to stream the data. Use Amazon Kinesis Data Analytics to transform the data. Use Amazon Kinesis Data Firehose to write the data to Amazon S3.
...
**Correct Answer:** D. Use AWS Storage Gateway and configure a stored volume gateway. Run the Storage Gateway software appliance on premises and map the gateway storage volumes to on-premises storage. Mount the gateway storage volumes to provide local access to the data.
...
**Correct Answer:** C. Migrate the application layer to Amazon EC2 Reserved Instances. Migrate the data storage layer to Amazon Aurora Reserved Instances.
...
<a id="questions-301-to-350"></a>

## Questions 301 to 350

### Q301. `Storage`

301Topic 1 A university research laboratory needs to migrate 30 TB of data from an on-premises Windows file server to Amazon FSx for Windows File Server. The laboratory has a 1 Gbps network link that many other departments in the university share. The laboratory wants to implement a data migration service that will maximize the performance of the data transfer. However, the laboratory needs to be able to control the amount of bandwidth that the service uses to minimize the impact on other departments. The data migration must take place within the next 5 days. Which AWS solution will meet these requirements?
...
**Correct Answer:** A. Launch all EC2 instances in the same Availability Zone within the same AWS Region. Specify a placement group with cluster strategy when launching EC2 instances.
...
**Simple Explanation:**
> AWS Storage Gateway provides a hybrid cloud storage service that enables on-premises applications to use cloud storage seamlessly. Volume Gateway offers two modes: cached volumes and stored volumes. In the cached volumes mode, the entire dataset is stored in Amazon S3, and the most frequently accessed data is cached on-premises.
...
**Correct Answer:** A. Analyze bucket access patterns by using the S3 Storage Lens dashboard for advanced activity metrics.
...
**Correct Answer:** B. Deploy an Amazon CloudFront distribution with the existing S3 bucket as the origin. Direct customer requests to the CloudFront URL. Switch to CloudFront signed URLs for access control.

**Simple Explanation:**
> Amazon CloudFront: CloudFront is a content delivery network (CDN) service that distributes content globally with low latency and high data transfer speeds. It helps reduce data transfer costs and improves performance by caching content at edge locations. S3 Bucket as the Origin: By configuring the existing S3 bucket as the origin for CloudFront, you allow CloudFront to cache and serve the datasets from edge locations around the world.
...
**Correct Answer:** C. Create a backup plan by using AWS Backup to perform nightly backups. Copy the backups to another Region. Add the application’s EBS volumes as resources.

**Simple Explanation:**
> AWS Backup: AWS Backup is a fully managed backup service that centralizes and automates the backup of data across AWS services. It provides a simple and efficient way to back up your EC2 instances and their associated EBS volumes. Backup Plan: With AWS Backup, you can create backup plans to define when and how your backups are performed.
...
**Correct Answer:** B. Amazon Aurora Serverless for MySQL
...
**Simple Explanation:**
> AWS Lambda: Lambda is a serverless computing service that allows you to run code without provisioning or managing servers. It automatically scales based on the number of incoming requests. Cost-Efficiency: With Lambda, you only pay for the compute time consumed during code execution.
...
**Correct Answer:** A. Enable AWS CloudTrail and use it for auditing.
...
**Correct Answer:** A. Publish data to Amazon Kinesis Data Streams, Use Kinesis Data Analytics to query the data.
...
**Correct Answer:** C. Create an Amazon Simple Queue Service (Amazon SQS) message queue. As images are uploaded, place a message on the SQS queue for thumbnail generation. Alert the user through an application message that the image was received.
...
**Correct Answer:** B. Create an HTTPS endpoint in Amazon API Gateway. Configure the API Gateway endpoint to invoke an AWS Lambda function to process the messages and save the results to an Amazon DynamoDB table.
...
**Correct Answer:** A. Move assets to S3 Intelligent-Tiering after 30 days.
...
**Correct Answer:** B. Add an Amazon CloudFront distribution for the static content. Place the EC2 instances in an Auto Scaling group to launch new instances based on network traffic.

**Simple Explanation:**
> Amazon CloudFront for Static Content: By using CloudFront, you can distribute static content (like images, stylesheets) globally, reducing latency for end-users and offloading some of the traffic from your backend instances. Auto Scaling Group: An Auto Scaling group allows you to automatically adjust the number of EC2 instances to handle changes in demand.
...
**Correct Answer:** A. Create a key in AWS Key Management Service (AWS KMS). Enable encryption for the DB instances.
...
**Correct Answer:** A. Use AWS Snowball.
...
**Correct Answer:** C. Configure an EC2 Auto Scaling scheduled scaling policy based on the monthly schedule.
...
**Correct Answer:** B. Enable Amazon Elastic Block Store (Amazon EBS) fast snapshot restore on a snapshot. Provision an AMI by using the snapshot. Replace the AMI in the Auto Scaling group with the new AMI.
...
**Correct Answer:** A. Create a new AWS Key Management Service (AWS KMS) encryption key. Use AWS Secrets Manager to create a new secret that uses the KMS key with the appropriate credentials. Associate the secret with the Aurora DB cluster. Configure a custom rotation period of 14 days.
...
**Correct Answer:** A. Migrate the database to Amazon Aurora MySQL. Replace the read replicas with Aurora Replicas, and configure Aurora Auto Scaling. Replace the stored procedures with Aurora MySQL native functions.

**Simple Explanation:**
> Amazon Aurora MySQL: Aurora Replicas in Amazon Aurora MySQL are designed to have minimal replication lag compared to traditional MySQL read replicas. Aurora is built for high performance and low replication lag, making it a suitable choice for reducing lag in read replicas. Aurora Auto Scaling: Aurora Auto Scaling allows you to automatically adjust the number of Aurora Replicas based on actual application usage.
...
**Correct Answer:** D. Set up an Aurora global database for the DB cluster. Specify a minimum of one DB instance in the secondary Region.
...
**Simple Explanation:**
> AWS WAF (Web Application Firewall): AWS WAF is designed to protect web applications from common web exploits, including SQL injection. It allows you to create web access control lists (web ACLs) to define rules that filter and monitor HTTP traffic to your application. Associating Web ACLs with AWS WAF: By using AWS WAF in front of the ALB, you can define rules to block or allow web requests based on conditions that you specify.
...
**Correct Answer:** A. Use Amazon Cognito for authentication. Use Lambda@Edge for authorization. Use Amazon CloudFront to serve the web application globally.

**Simple Explanation:**
> Amazon Cognito for Authentication: Amazon Cognito is a fully managed service for user identity and access control. It provides easy integration for authentication with a serverless architecture and supports a user pool for fewer than 100 users. Lambda@Edge for Authorization: Lambda@Edge allows you to run custom code in response to CloudFront events, including authorization.
...
**Correct Answer:** B. Create a database snapshot. Add the acquiring company’s AWS account to the KMS key policy. Share the snapshot with the acquiring company’s AWS account.
...
**Correct Answer:** A. Modify the DB instance from a Single-AZ DB instance to a Multi-AZ deployment:

**Simple Explanation:**
> Enabling Multi-AZ deployment provides high availability by replicating the database to a standby instance in another Availability Zone. This helps in automatic failover and recovery in case of a primary instance failure. By creating a read replica in a different Availability Zone, you offload the reporting workload from the primary instance, reducing the impact on transaction processing.
...
<a id="questions-351-to-400"></a>

## Questions 351 to 400
...
**Correct Answer:** A. Store the static files on Amazon S3. Use Amazon CloudFront to cache objects at the edge.

**Simple Explanation:**
> Amazon S3 is a highly scalable and durable object storage service, and it is well-suited for storing static files. Using CloudFront as a content delivery network (CDN) improves the delivery of static content by caching objects at edge locations, reducing latency for end users. Amazon FSx for Windows File Server provides a fully managed Windows file system that is accessible from Windows-based EC2 instances.
...
**Correct Answer:** B. Turn on server-side encryption on the SNS components by using an AWS Key Management Service (AWS KMS) customer managed key. Apply a key policy to restrict key usage to a set of authorized principals.

**Simple Explanation:**
> This option ensures that data at rest in the SNS components is encrypted using an AWS KMS customer managed key. The key policy restricts key usage to authorized personnel. This option ensures that data at rest in the SQS components is encrypted using an AWS KMS customer managed key.
...
**Simple Explanation:**
> Amazon RDS (Relational Database Service) can automatically create backups of your database every day. These backups are like snapshots of your entire database, capturing all the data. They happen automatically, so you don't have to remember to do it.
...
**Correct Answer:** A. Configure three Network Load Balancers (NLBs) in the three AWS Regions to address the on-premises endpoints. Create an accelerator by using AWS Global Accelerator, and register the NLBs as its endpoints. Provide access to the application by using a CNAME that points to the accelerator DNS.
...
**Simple Explanation:**
> Amazon Web Services (AWS) allows you to set an account-wide password policy using AWS Identity and Access Management (IAM). This policy defines the rules and requirements for all IAM users in the AWS account. It's a centralized approach to enforce security measures consistently across all users.
...
**Correct Answer:** A. Use AWS Batch to run the tasks as jobs. Schedule the jobs by using Amazon EventBridge (Amazon CloudWatch Events).
...
**Correct Answer:** C. Provision a NAT gateway in a public subnet. Modify each private subnet's route table with a default route that points to the NAT gateway.

**Simple Explanation:**
> NAT Gateway: A NAT gateway is a managed service provided by AWS that allows EC2 instances in private subnets to initiate outbound traffic to the internet while preventing unsolicited inbound traffic from reaching those instances. NAT gateways are fully managed, highly available, and require minimal maintenance. Public Subnet: Placing the NAT gateway in a public subnet allows it to have access to the internet, fulfilling the requirement for private instances to communicate with a license server over the internet.
...
**Correct Answer:** C. Enable EBS encryption by default in the AWS Region where the EKS cluster will be created. Select the customer managed key as the default key.

**Simple Explanation:**
> EBS encryption is set regionally. AWS account is global but it does not mean EBS encryption is enable by default at account level. default EBS encryption is a regional setting within your AWS account.
...
**Correct Answer:** D. Store the images in Amazon S3 buckets. Store geographic codes and image S3 URLs in a database table. Use Oracle running on an Amazon RDS Multi-AZ DB instance.
...
**Correct Answer:** A. Create a proxy in RDS Proxy. Configure the users’ applications to use the DB instance through RDS Proxy.
...
**Correct Answer:** B. Configure provisioned concurrency for the Lambda function that handles the requests.
...
**Correct Answer:** D. Create an AWS Lambda function that will start and stop the EC2 instances and DB instances. Configure Amazon EventBridge to invoke the Lambda function on a schedule.
...
**Correct Answer:** B. Set up a new Amazon Aurora PostgreSQL DB cluster that includes an Aurora Replica. Issue queries to the Aurora Replica to generate the reports.
...
**Correct Answer:** A. Configure a TLS listener. Deploy the server certificate on the NLB.
...
**Correct Answer:** A. Dedicated Reserved Hosts
...
**Simple Explanation:**
> Amazon EFS provides scalable and highly available file storage in the cloud. The Standard storage class is designed for frequently accessed data, making it suitable for the initial 30 days of frequent access. You can create a lifecycle management policy for EFS that automatically transitions infrequently accessed files to the EFS Standard-Infrequent Access (EFS Standard-IA) storage class.
...
**Correct Answer:** C. Create a security group for the web servers and allow port 443 from the load balancer. Create a security group for the MySQL servers and allow port 3306 from the web servers security group.
...
**Correct Answer:** D. Create a new IAM user for the deployment engineer and add the IAM user to a group that has an IAM policy that allows AWS CloudFormation actions only.

**Simple Explanation:**
> This ensures that the IAM user has the necessary permissions for AWS CloudFormation but not unnecessary permissions for other AWS services. IAM roles are more suitable for temporary elevated permissions needed during AWS CloudFormation stack operations. The deployment engineer can assume the role when required, limiting their permissions to only what is needed for those specific actions.
...
**Correct Answer:** D. Add an inbound rule to the security group of the database tier’s RDS instance to allow traffic from the web tiers security group.
...
**Correct Answer:** A. Deploy RDS read replicas to process the business reporting queries.
...
**Correct Answer:** C. Configure an Amazon Transcribe transcription job with PII redaction turned on. When an audio file is uploaded to the S3 bucket, invoke an AWS Lambda function to start the transcription job. Store the output in a separate S3 bucket.
...
**Correct Answer:** A. Subscribe to AWS Shield Advanced. Add the accelerator as a resource to protect.
...
**Correct Answer:** C. Use the AWS Snow Family console to order several AWS Snowball Edge Storage Optimized devices. Use the devices to transfer the data to Amazon S3.
...
**Correct Answer:** A. Deploy the application servers by using Amazon EC2 instances in an Auto Scaling group across multiple Availability Zones. Use an Amazon RDS DB instance in a Multi-AZ configuration.
...
**Correct Answer:** C. Create a security group for the web servers in the public subnet. Add a rule to allow traffic from 0.0.0.0/0 on port 443.

**Simple Explanation:**
> This allows inbound traffic from the internet on port 443 to the web servers. This ensures that the RDS instance is accessible only from the web servers in the public subnet.
...
**Correct Answer:** B. Use AWS Global Accelerator. Create a Network Load Balancer (NLB) in each of the two Regions as an endpoint. Create an Amazon Elastic Container Service (Amazon ECS) cluster with the Fargate launch type. Create an ECS service on the cluster. Set the ECS service as the target for the NLProcess the data in Amazon ECS.

**Simple Explanation:**
> AWS Global Accelerator: AWS Global Accelerator provides static IP addresses that act as a fixed entry point to your application. It routes traffic over the AWS global network to the optimal AWS endpoint based on health, geography, and routing policies. Network Load Balancer (NLB): NLB is well-suited for UDP-based traffic, and it's designed for high-performance, low-latency applications.
...
**Correct Answer:** B. Create the EBS volumes as encrypted volumes. Attach the EBS volumes to the EC2 instances.
...
**Correct Answer:** A. Create an S3 Lifecycle configuration with a rule to transition the objects in the S3 bucket to S3 Intelligent-Tiering.
...
**Correct Answer:** B. Set up an Amazon CloudFront distribution.

**Simple Explanation:**
> Amazon CloudFront is a content delivery network (CDN) that can improve the performance of a website by caching static content closer to the users. This reduces latency and improves page load times. Configure CloudFront to distribute static content such as images, stylesheets, and JavaScript files.
...
**Correct Answer:** C. Create an SCP. Attach the SCP to the root organizational unit (OU). Define the SCP to deny the ec2:CreateVolume action whenthe ec2:Encrypted condition equals false.
...
**Correct Answer:** D. Use an Amazon RDS Multi-AZ DB cluster deployment Point the read workload to the reader endpoint.
...
**Correct Answer:** B. Create an encrypted Amazon Elastic File System (Amazon EFS) volume. Create an AWS Transfer Family SFTP service with elastic IP addresses and a VPC endpoint that has internet-facing access. Attach a security group to the endpoint that allows only trusted IP addresses. Attach the EFS volume to the SFTP service endpoint. Grant users access to the SFTP service.
...
**Correct Answer:** B. Use Reserved Instances for the frontend nodes. Use Spot Instances for the backend nodes.
...
**Correct Answer:** C. GP3 volume type
...
**Correct Answer:** D. Use AWS Storage Gateway to move the existing data to Amazon S3. Use AWS CloudTrail to log management events.
...
**Correct Answer:** B. Design an AWS Lambda function that converts the .csv files into images and stores the images in the S3 bucket. Invoke the Lambda function when a .csv file is uploaded. Most Voted
...
**Correct Answer:** B. Use Amazon SageMaker to build and train models. Use Amazon QuickSight to visualize the data.
...
**Correct Answer:** A. Order an AWS Snowball Edge Storage Optimized device. Use AWS Database Migration Service (AWS DMS) with AWS Schema Conversion Tool (AWS SCT) to migrate the database with replication of ongoing changes. Send the Snowball Edge device to AWS to finish the migration and continue the ongoing replication.

**Simple Explanation:**
> This is a cost-effective solution for shipping large amounts of data to AWS. Snowball Edge devices are designed for efficient data transfer, and they can handle the 20 TB database. AWS DMS is a managed service for migrating databases to AWS, and AWS SCT can assist in converting the database schema.
...
**Correct Answer:** A. Buy reserved DB instances for the total workload. Make the Amazon RDS for PostgreSQL DB instance larger.
...
**Correct Answer:** D. Create an encrypted snapshot of the database. Share the snapshot with the auditor. Allow access to the AWS Key Management Service (AWS KMS) encryption key.
...
**Correct Answer:** A. Add an additional IPv4 CIDR block to increase the number of IP addresses and create additional subnets in the VPC. Create new resources in the new subnets by using the new CIDR.
...
**Correct Answer:** A. Import the RDS snapshot directly into Aurora.
...
**Correct Answer:** A. Use Amazon S3 with Transfer Acceleration to host the application.
...
**Correct Answer:** B. Update the DB instance to be Multi-AZ, and enable deletion protection. Place the EC2 instances behind an Application Load Balancer, and run them in an EC2 Auto Scaling group across multiple Availability Zones.
...
**Correct Answer:** D. Turn on S3 Object Lock with compliance retention mode for the S3 bucket. Set the retention period to expire after 7 years. Use S3 Batch Operations to bring the existing data into compliance.
...
**Correct Answer:** A. Create Amazon Route 53 health checks for each Region. Use an active-active failover configuration.
...
**Correct Answer:** C. Create a VPC across two Availability Zones. Refactor the application to host the web tier, application tier, and database tier. Host each tier on its own private subnet with Auto Scaling groups for the web tier and application tier.

**Simple Explanation:**
> This choice aligns with best practices by using separate subnets for each tier, allowing for better security and scalability. Auto Scaling groups provide elasticity and resiliency. This option introduces an Elastic Load Balancer (ELB) for the web tier, which enhances scalability and resiliency.
...
<a id="questions-451-to-500"></a>

## Questions 451 to 500
...
**Correct Answer:** B. Copy the code into an AWS Lambda function that has 1 GB of memory. Create an Amazon EventBridge scheduled rule to run the code each hour.

**Simple Explanation:**
> AWS Lambda is a serverless compute service that allows you to run code without provisioning or managing servers. It automatically scales based on the number of requests, making it cost-effective for sporadic workloads. Scheduled Rule with Amazon EventBridge: Amazon EventBridge allows you to schedule events at specified intervals.
...
**Correct Answer:** C. Use Workload Discovery on AWS to generate architecture diagrams of the workloads.
...
**Correct Answer:** B. Use AWS Budgets to create a budget. Set the budget amount under the Billing dashboards of the required AWS accounts.
...
**Correct Answer:** A. From the Organizations management account billing console, activate a user-defined cost allocation tag named department. Create one cost report in Cost Explorer grouping by tag name, and filter by EC2.
...
**Simple Explanation:**
> Amazon AppFlow is a fully managed integration service that allows you to securely transfer data between AWS services and SaaS applications like Salesforce. It supports data encryption both in transit and at rest. With AppFlow, you can configure the integration flow, including source (Salesforce) and destination (Amazon S3), and set up encryption options.
...
**Correct Answer:** B. Use AWS Global Accelerator to create an accelerator. Create a Network Load Balancer (NLB) behind an accelerator endpoint that uses Global Accelerator integration and listening on the TCP and UDP ports. Update the Auto Scaling group to register instances on the NLB.
...
**Correct Answer:** C. Use AWS Lambda with a Python script

**Simple Explanation:**
> AWS Lambda is a serverless compute service that allows you to run code without provisioning or managing servers. It automatically scales with the number of requests, making it well-suited for event-driven workloads like processing data from IoT devices. Python is a lightweight and efficient language for data processing tasks.
...
**Correct Answer:** A. Convert the existing database instance to a Multi-AZ deployment by modifying the database instance and specifying the Multi-AZ option.
...
**Correct Answer:** C. Use Provisioned IOPS SSD (io2) EBS volumes with Amazon Elastic Block Store (Amazon EBS) Multi-Attach
...
**Correct Answer:** B. Turn on discount sharing from the Billing Preferences section of the account console in the company's Organizations management account.
...
**Correct Answer:** B. Design a REST API by using Amazon API Gateway. Host the application in Amazon Elastic Container Service (Amazon ECS) in a private subnet. Create a private VPC link for API Gateway to access Amazon ECS.
...
**Correct Answer:** C. Create a gateway VPC endpoint for Amazon S3. Associate this endpoint with all route tables in the VPC.
...
**Correct Answer:** A. Configure Amazon DynamoDB Accelerator (DAX) for the new messages table. Update the code to use the DAX endpoint.
...
**Correct Answer:** A. Create an Amazon CloudFront distribution to cache state files at edge locations
...
**Correct Answer:** C. Use AWS Transit Gateway to manage VPC communication in a single Region and Transit Gateway peering across Regions to manage VPC communications.
...
**Correct Answer:** C. Amazon Elastic File System (Amazon EFS) with the Standard storage class
...
**Correct Answer:** C. Create an IAM policy that grants least privilege permission. Attach the policy to the IAM groups
...
**Correct Answer:** B. Use AWS DataSync to migrate the data from the on-premises location to an S3 bucket
...
**Correct Answer:** A. Create a new organization in AWS Organizations with all features turned on. Create the new AWS accounts in the organization.
...
**Correct Answer:** A. Store the video archives in Amazon S3 Glacier and use Expedited retrievals.
...
**Correct Answer:** A. Use Amazon S3 to host static content. Use Amazon Elastic Container Service (Amazon ECS) with AWS Fargate for compute power. Use a managed Amazon RDS cluster for the database.

**Simple Explanation:**
> Amazon S3 is a highly scalable and cost-effective storage service that can be used to host static content like a static website. It simplifies the storage and delivery of static assets. AWS Fargate is a serverless compute engine for containers.
...
**Correct Answer:** C. Amazon Elastic File System (Amazon EFS) with multiple mount targets
...
**Correct Answer:** C. Create a service control policy (SCP) to deny access to the billing information. Attach the SCP to the root organizational unit (OU).
...
**Correct Answer:** C. Configure an Amazon SNS dead letter queue that has an Amazon Simple Queue Service (Amazon SQS) target with a retention period of 14 days.
...
**Correct Answer:** B. Use AWS Organizations to organize the accounts into organizational units (OUs). Define and attach a service control policy (SCP) to control the usage of EC2 instance types.

**Simple Explanation:**
> AWS Organizations allows you to consolidate multiple AWS accounts into an organization that you create and centrally manage. Organizational Units (OUs) can be used to group accounts based on different criteria, such as development, production, etc. Service Control Policies (SCPs) are used to set fine-grained permissions on AWS accounts within an organization.
...
**Correct Answer:** D. Use Amazon Transcribe to convert the audio recordings in any language into text.

**Simple Explanation:**
> Use Amazon Transcribe to Convert Audio Recordings into Text: Amazon Transcribe is a service that converts speech into text. Use it to transcribe the customer service call recordings into text. Use Amazon Translate to Translate Text into English: Amazon Translate is a service that provides language translation.
...
**Correct Answer:** B. Deploy an AWS Storage Gateway file gateway to replace NFS storage.
...
**Correct Answer:** C. Provision a VPC gateway endpoint. Configure the route table for the private subnet to use the gateway endpoint as the route for all S3 traffic.
...
**Correct Answer:** D. Contact an AWS Direct Connect Partner to order a 200 Mbps hosted connection for an existing AWS account.
...
**Correct Answer:** A. Deploy AWS DataSync agents on premises. Schedule DataSync tasks to transfer the data to the FSx for Windows File Server file system.

---

<a id="questions-501-to-550"></a>

## Questions 501 to 550
...
**Correct Answer:** C. Use Amazon Kinesis Data Firehose to ingest data. Use Amazon Kinesis Data Analytics to analyze the data in real time.

**Simple Explanation:**
> Amazon Kinesis Data Firehose: It is a fully managed service that can reliably load streaming data into data lakes, data stores, and analytics tools. It can automatically scale to handle varying data throughput. It simplifies the data delivery process, making it easy to ingest data into Amazon S3.
...
**Correct Answer:** C. Move the website images onto an Amazon Elastic File System (Amazon EFS) file system that is mounted on every EC2 instance.
...
**Correct Answer:** A. Ensure that the customers create an IAM role in their account with read-only EC2 and CloudWatch permissions and a trust policy to the company’s account.
...
**Correct Answer:** C. Create a new launch template for the Auto Scaling group. Set the instances to Spot Instances. Set a policy to scale out based on CPU usage.
...
**Correct Answer:** C. Generate Amazon S3 presigned URLs in the application. Upload files directly from the user's browser into an S3 bucket.
...
Topic 1 A company has migrated multiple Microsoft Windows Server workloads to Amazon EC2 instances that run in the us-west-1 Region. The company manually backs up the workloads to create an image as needed. In the event of a natural disaster in the us-west-1 Region, the company wants to recover workloads quickly in the us-west-2 Region. The company wants no more than 24 hours of data loss on the EC2 instances. The company also wants to automate any backups of the EC2 instances. Which solutions will meet these requirements with the LEAST administrative effort? (Choose two.)?

**Correct Answer:** B. Create an Amazon EC2-backed Amazon Machine Image (AMI) lifecycle policy to create a backup based on tags. Schedule the backup to run twice daily. Configure the copy to the us-west-2 Region.
...
**Correct Answer:** B. Modify the network ACL for the web tier subnets. Add an inbound deny rule for the IP addresses that are consuming resources.
...
**Correct Answer:** C. Configure a VPC peering connection between the ap-southeast-2 VPC and the eu-west-1 VPUpdate the subnet route tables. Create an inbound rule in the ap-southeast-2 database security group that allows traffic from the eu-west-1 application server IP addresses.
...
**Simple Explanation:**
> Amazon Aurora is designed for high-performance, scalability, and availability. It offers features such as automatic failover, replication, and performance enhancements over traditional PostgreSQL. On-Demand pricing means you pay for the resources you consume, making it a flexible option.
...
**Correct Answer:** A. Use AWS Config to identify all untagged resources. Tag the identified resources programmatically. Use tags in the backup plan.
...
**Simple Explanation:**
> Hosting a static website in Amazon S3 is a cost-effective and highly available solution. Amazon S3 provides scalable and durable object storage. A static website in S3 can serve as the front end for user interactions.
...
**Correct Answer:** B. Create interface VPC endpoints to allow nodes to access the control plane.
...
**Correct Answer:** B. Supporting client-side and server-side encryption

**Simple Explanation:**
> Amazon Redshift supports encryption for data at rest and in transit, providing security features for sensitive data. This makes it suitable for scenarios where encryption is a requirement. Amazon Redshift is a fully managed data warehouse service optimized for analytical queries.
...
**Correct Answer:** B. Configure DynamoDB in on-demand mode by using the DynamoDB Standard table class.
...
**Correct Answer:** B. Use the Kubernetes Metrics Server for Horizontal Pod Autoscaling:
...
**Correct Answer:** B. Amazon CloudFront with Lambda@Edge functions

**Simple Explanation:**
> Amazon CloudFront is a content delivery network (CDN) service that can distribute content globally with low latency. Lambda@Edge allows you to run custom code in response to CloudFront events, such as viewer requests, origin requests, and more. By using Lambda@Edge functions, you can customize and augment the behavior of CloudFront.
...
**Correct Answer:** A. Access usage cost-related data by using the AWS Cost Explorer API with pagination.
...
**Correct Answer:** C. Add AWS Global Accelerator in front of the NLBs. Configure a Global Accelerator endpoint to use the correct listener ports.
...
**Simple Explanation:**
> AWS Lambda supports API Gateway integration, which allows you to create an HTTP endpoint (URL) for your Lambda function. Operational Efficiency: Directly exposing the Lambda function through a URL eliminates the need for additional services, such as load balancers or message queues, for simple webhook integration. Simplicity: This approach is straightforward and easy to implement.
...
**Correct Answer:** A. Register the required domain in a registrar. Create a wildcard custom domain name in a Route 53 hosted zone and record in the zone that points to the API Gateway endpoint.

**Simple Explanation:**
> Registering the domain in a registrar and creating a wildcard custom domain name in Route 53 allows you to manage the DNS records efficiently. The DNS records can point to the API Gateway endpoint. ACM provides a simple way to request and manage SSL/TLS certificates.
...
**Correct Answer:** B. Transition objects to the S3 Standard-Infrequent Access (S3 Standard-IA) storage class 30 days after creation. Move all objects to the S3 Glacier Flexible Retrieval storage class after 90 days. Write an expiration action that directs Amazon S3 to delete objects after 90 days.
...
**Correct Answer:** B. Create a new AWS Key Management Service (AWS KMS) key. Enable Amazon EKS KMS secrets encryption on the Amazon EKS cluster.
...
**Correct Answer:** C. Change the setup from a Single-AZ to a Multi-AZ instance deployment. Provide two additional read replicas for the data scientists.
...
**Correct Answer:** A. Add geographic restrictions to the content in CloudFront by using an allow list. Set up a custom error message.
...
**Correct Answer:** D. Use Amazon RDS deployed in a Multi-AZ instance deployment to create an Amazon Aurora database. Direct the reporting functions to the reader instances.
...
**Correct Answer:** A. Create an AWS Lambda function to retrieve user information from Amazon DynamoDB. Create an Amazon API Gateway endpoint to accept RESTful APIs. Send the API calls to the Lambda function.

**Simple Explanation:**
> AWS Lambda is a serverless computing service, and its pay-per-use pricing model can be cost-effective for sporadic and unpredictable workloads. DynamoDB is a NoSQL database that can scale with demand. Amazon Cognito provides a scalable and secure user directory for your web application.
...
**Correct Answer:** A. Create a canary release deployment stage for API Gateway. Deploy the latest API version. Point an appropriate percentage of traffic to the canary stage. After API verification, promote the canary stage to the production stage.
...
**Correct Answer:** A. Use Amazon Kinesis Data Firehose to deliver streaming data to Amazon S3.
...
**Correct Answer:** B. Add AWS KMS permissions in the Lambda execution role.

**Simple Explanation:**
> The Lambda execution role is the role assumed by the Lambda function when it runs. It needs permissions to use the KMS key to decrypt the environment variables. Grant the kms:Decrypt permission on the specific KMS key used for encryption to the Lambda execution role.
...
<a id="questions-551-to-600"></a>

## Questions 551 to 600
...
**Correct Answer:** A. Use S3 Standard. Use an S3 Lifecycle rule to transition the reports to S3 Glacier after 7 days.
...
**Correct Answer:** B. Purchase a No Upfront Compute Savings Plan for a 1-year term.
...
**Correct Answer:** A. Implement an interface VPC endpoint for Amazon SQS. Configure the endpoint to use the private subnets. Add to the endpoint a security group that has an inbound access rule that allows traffic from the EC2 instances that are in the private subnets.
...
**Simple Explanation:**
> By creating an IAM role with the necessary permissions to access DynamoDB and associating it with the EC2 instance profile, you can securely grant permissions to the EC2 instances without exposing API credentials in the CloudFormation template.
...
**Correct Answer:** B. Use Amazon EMR to process the S3 data. Use Amazon EMR with the Amazon Redshift data to enrich the S3 data.
...
**Correct Answer:** C. Set up a VPC peering connection between the VPCs. Update the route tables of each VPC to use the VPC peering connection for inter-VPC communication.
...
**Correct Answer:** B. Select a specific user-defined tag in the AWS Billing console.

**Simple Explanation:**
> User-defined tags are tags that you create and attach to your AWS resources. In this case, since teams for each product line have tagged each compute resource with user-defined tags, selecting a specific user-defined tag in the AWS Billing console allows you to filter costs based on those tags. The consolidated billing feature in AWS Organizations allows you to view and manage costs across multiple AWS accounts.
...
**Correct Answer:** A. Provision the AWS accounts by using AWS Control Tower. Use account drift notifications to identify the changes to the OU hierarchy.

**Simple Explanation:**
> AWS Control Tower is a service that simplifies the process of setting up and governing a secure, multi-account AWS environment based on AWS best practices. It provides a pre-defined landing zone with an organizational structure, OUs, and guardrails to enforce security and compliance. the organizational units (OUs) are established as part of the AWS Control Tower landing zone.
...
**Correct Answer:** A. Create a route table entry for the endpoint.
...
**Simple Explanation:**
> AWS DMS is a fully managed service that helps you migrate databases to AWS easily and securely. It supports homogeneous and heterogeneous database migrations. Amazon Aurora is a fully managed relational database service that is compatible with MySQL and PostgreSQL.
...
**Correct Answer:** A. Use Amazon API Gateway with AWS Lambda functions to receive the data from the sensors, process the data, and store the data in an Amazon DynamoDB table.
...
**Correct Answer:** A. Use a local machine to create a certificate that is signed by the third-party CImport the certificate into AWS Certificate Manager (ACM). Create an HTTP API in Amazon API Gateway with a custom domain. Configure the custom domain to use the certificate.
...
**Correct Answer:** C. Create an Amazon RDS database with Multi-AZ DB cluster deployment.
...
**Correct Answer:** C. Use AWS Certificate Manager (ACM) to create a certificate. Use DNS validation for the domain.

**Simple Explanation:**
> AWS Certificate Manager (ACM): ACM is a fully managed service that allows you to easily provision, manage, and deploy public and private Secure Sockets Layer/Transport Layer Security (SSL/TLS) certificates for use with AWS services and your internal connected resources. It is designed for automation and ease of use. DNS Validation: DNS validation involves adding a DNS record to your domain's DNS configuration.
...
**Correct Answer:** D. Purchase a Capacity Reservation in the failover Region.
...
**Correct Answer:** B. Invite the R&D AWS account to be part of the new organization after the R&D AWS account has left the prior organization.
...
**Simple Explanation:**
> This is a Kubernetes-native controller that allows you to define and manage Application Load Balancers and Network Load Balancers to route traffic to services in your Amazon EKS cluster. ALBs are designed for routing HTTP/HTTPS traffic and provide more advanced routing features compared to Network Load Balancers.
...
**Correct Answer:** A. Deploy an AWS Storage Gateway on premises in Amazon S3 File Gateway mode.

**Simple Explanation:**
> This step allows you to seamlessly integrate on-premises devices with AWS S3, providing a scalable and cost-effective storage solution. The AWS Storage Gateway in S3 File Gateway mode enables you to write data from on-premises devices to S3. AWS Glue can discover, catalog, and transform data from S3.
...
<a id="questions-601-to-650"></a>

## Questions 601 to 650
...
**Simple Explanation:**
> AWS Backup: AWS Backup is a fully managed backup service that centralizes and automates the backup of data across AWS services. Backup Plan: You can set up a backup plan in AWS Backup to create and manage backups of the entire group of EC2 instances. AWS Backup provides a streamlined process for restoring data.
...
**Correct Answer:** B. Store all the objects in S3 Standard with an S3 Lifecycle rule to transition the objects to S3 Standard-Infrequent Access (S3 Standard-IA) after 30 days.
...
**Correct Answer:** C. Create an Amazon S3 bucket. Update the application to store documents in the S3 bucket. Store the object metadata in the existing database.
...
**Simple Explanation:**
> Kinesis Data Streams is designed for ingesting and processing real-time streaming data at scale. It can handle large volumes of data and provides the ability to scale horizontally. Using Lambda functions allows for serverless, event-driven processing of the data.
...
**Simple Explanation:**
> Amazon GuardDuty: GuardDuty is a threat detection service that continuously monitors for malicious activity and unauthorized behavior in your AWS account. It analyzes events, such as API calls and network traffic, to detect potentially malicious activity. AWS Security Hub: AWS Security Hub is a comprehensive security service that aggregates and prioritizes security findings from various AWS services, including GuardDuty.
...
**Simple Explanation:**
> FSx for Windows File Server: Create an FSx for Windows File Server file system in the us-east-1 Region with a Multi-AZ deployment type. The Multi-AZ deployment type ensures high availability. AWS Backup: Use AWS Backup to create a daily backup plan for the FSx file system.
...
**Correct Answer:** C. Deploy Amazon DynamoDB as the database solution. Ensure that DynamoDB auto scaling is enabled.

**Simple Explanation:**
> DynamoDB auto scaling allows the database to automatically adjust its read and write capacity based on the application's traffic, making it well-suited for varying workloads. Amazon S3 is a highly scalable and durable object storage service, and using CloudFront, a content delivery network (CDN), helps distribute static content globally, reducing latency and providing scalability.
...
**Correct Answer:** C. Create an SNS subscription that sends the event to Amazon Simple Queue Service (Amazon SQS). Configure the SOS queue to trigger a Lambda function.
...
**Correct Answer:** B. Grant the decrypt permission for the Lambda IAM role in the KMS key's policy
...
**Correct Answer:** B. Enable Cost and Usage Reports in the management account. Deliver the reports to Amazon S3 Use Amazon Athena for analysis.
...
**Correct Answer:** A. Store the logs in Amazon S3. Use Amazon Athena tor analysis.
...
**Correct Answer:** A. Use the AWS Certificate Manager (ACM) console to request a public certificate for the apex top domain example com and a wildcard certificate for *.example.com.
...
**Correct Answer:** C. Configure the General Purpose SSD (gp3) EBS volume storage type and provision 15,000 IOPS.
...
**Correct Answer:** wer: B) Configure a transient cluster with primary/core nodes on On-Demand Instances and task nodes on Spot Instances.

**Simple Explanation:**
> Transient clusters are cost-effective for short workloads. Spot Instances reduce costs for non-critical task nodes. Long-running clusters (Options A/D) are unnecessary for a 6-hour workload.
...
**Correct Answer:** wer: B) Create a Lambda function triggered by EventBridge (via CloudTrail) to tag resources based on the RDS cost center DB.
...
**Correct Answer:** wer: D) Use CloudFront + S3 for static content, ALB + ECS Fargate for PHP, and Multi-AZ ElastiCache for Redis.
...
**Correct Answer:** wers: C) Create a public ALB + E) Associate a WAF web ACL with the endpoint.

**Simple Explanation:**
> ALB supports sticky sessions (unlike NLB/GWLB). WAF provides security. Elastic IPs (Option D) are not scalable.
...
**Correct Answer:** wer: D) Store images in S3 Standard-IA and deliver via static website.
...
**Correct Answer:** wer: B) Create a shared prefix list via AWS RAM and reference it in security groups.

**Simple Explanation:**
> Prefix lists centralize CIDR management. AWS RAM enables cross-account sharing. Firewall Manager (Option D) is overkill for CIDR updates.
...
**Correct Answer:** wers: A) Deploy compute-optimized EC2 in a cluster placement group + E) Use FSx for NetApp ONTAP.

**Simple Explanation:**
> Cluster placement minimizes latency. FSx for ONTAP supports multi-protocol (NFS/SMB) access. Lustre (Option C) lacks SMB support.
...
**Correct Answer:** wer: C) Use AWS Snowball Edge Storage Optimized.
...
**Correct Answer:** wer: D) Configure scheduled scaling to launch instances before peak hours.
...
**Correct Answer:** wer: B) Use Amazon RDS Proxy for connection pooling.
...
**Correct Answer:** wer: D) Delete nonessential snapshots + use Data Lifecycle Manager.

**Simple Explanation:**
> Automates snapshot retention per policy, reducing costs. Manual resizing (Options A/B) adds overhead.
...
**Correct Answer:** wer: C) Restrict S3/RDS access via VPC endpoints + security groups.
...
**Correct Answer:** wer: A) Use Elastic Beanstalk with burstable instances (unlimited mode) + request-based scaling.
...
**Correct Answer:** wer: B) Use AWS CloudFormation + AWS Config.
...
**Correct Answer:** wers: B) Migrate DB to RDS Multi-AZ + E) Use ALB + Auto Scaling for EC2.

**Simple Explanation:**
> RDS Multi-AZ ensures DB high availability. ALB + Auto Scaling distributes traffic across AZs. DynamoDB (Option C) requires code changes; DataSync (Option D) doesn’t solve HA.
...
**Correct Answer:** wer: A) Create S3 gateway endpoints.
...
**Correct Answer:** wer: D) Create a tag policy in Organizations with allowed application names.
...
**Correct Answer:** wer: C) Use Secrets Manager with RDS for automated password rotation.
...
**Correct Answer:** wer: B) Choose provisioned mode with calculated RCU/WCU.
...
**Correct Answer:** wer: B) Create a Cost Anomaly Detection monitor.
...
**Correct Answer:** wer: B) Use AWS Glue crawler + Athena for ad-hoc queries.

**Simple Explanation:**
> Glue catalogs data; Athena provides serverless SQL queries. EMR (Option C) adds operational overhead.
...
**Correct Answer:** wer: B) Use S3 File Gateway + Lifecycle policy to Glacier Deep Archive.
...
**Correct Answer:** wers: B) Create a read replica + D) Use ElastiCache for caching.

**Simple Explanation:**
> Read replicas offload read traffic. ElastiCache reduces DB load. Multi-AZ (Option C) doesn’t scale reads; auto scaling (Option A) isn’t for RDS.
...
**Correct Answer:** wer: D) Lock the EBS snapshots.
...
**Correct Answer:** wer: B) Use VPC Flow Logs → CloudWatch → Kinesis Firehose → OpenSearch.
...
**Correct Answer:** wer: B) Use mixed On-Demand + Spot Instances in managed node groups.

**Simple Explanation:**
> Balances cost (Spot) and reliability (On-Demand) for infrequent dev workloads. All-Spot (Option A) risks interruptions; self-managed ASG (Option C) adds overhead.
...
**Correct Answer:** wer: B) Use customer-managed KMS keys (SSE-KMS).
...
**Correct Answer:** wers: B) Enable versioning + C) Set 30-day retention + E) Expire objects after 30 days.

**Simple Explanation:**
> Versioning + retention policies automate compliance. Lifecycle rules expire objects. Object Lock (Option A) prevents deletion; tagging (Option F) doesn’t enforce retention.
...
**Correct Answer:** wer: A) Use DataSync with "changed data only" mode.

**Simple Explanation:**
> Continuously syncs only modified files to S3/EFS. Lambda (Option B) requires custom code; full syncs (Option C) are inefficient.
...
**Correct Answer:** wer: A) Use customer-managed KMS keys for EBS encryption.
...
**Correct Answer:** wer: A) Use IAM + AWS Config + Systems Manager for enforcement.

**Simple Explanation:**
> Config detects noncompliant volumes; Systems Manager automates remediation. Macie (Option C) is for data classification, not encryption.
...
**Correct Answer:** wers: A) Migrate web tier to ALB + Auto Scaling + C) Migrate DB to RDS Multi-AZ.

**Simple Explanation:**
> Auto Scaling improves web tier resiliency; RDS Multi-AZ ensures DB HA. Lambda (Option D) requires code changes; DynamoDB (Option E) is incompatible.
...
**Correct Answer:** wer: B) Deploy in AWS Local Zones.
...
<a id="questions-685-to-750"></a>

## BONUS: Latest Exam Questions (2026 Updated Topics)

> These **50 additional questions** cover the **newest and most frequently tested topics** on the SAA-C03 exam in 2026. Security is now **30% of the exam** (the largest domain), and disaster recovery, hybrid connectivity, and newer AWS services are heavily tested.

---

### Q685. A company needs to protect its VPC against network-level threats including intrusion attempts and malicious traffic. The company wants a managed solution that provides stateful inspection, intrusion prevention, and domain-based filtering without managing any infrastructure. Which solution meets these requirements? `Security`

**Correct Answer:** B. Deploy AWS Network Firewall in the VPC. Create firewall policies with stateful rules for intrusion prevention and domain filtering.

**Simple Explanation:**
> AWS Network Firewall is a managed firewall service for VPCs that provides stateful packet inspection, intrusion prevention/detection (IPS/IDS), and web filtering (domain allow/deny lists). It scales automatically with traffic and is fully managed, so there is no infrastructure to maintain. Security groups and NACLs cannot do deep packet inspection or domain filtering. Third-party appliances add operational overhead.

---

### Q686. A company is designing a disaster recovery (DR) strategy for a critical application. The application requires a Recovery Time Objective (RTO) of 10 minutes and a Recovery Point Objective (RPO) of near zero. Which DR strategy meets these requirements MOST cost-effectively? `Compute`

**Correct Answer:** C. Use a warm standby strategy with a scaled-down version of the production environment running in another Region. Use Amazon Aurora Global Database for near-zero RPO.

**Simple Explanation:**
> Warm standby runs a scaled-down but fully functional copy of your production environment in another Region. It provides minutes-level RTO (scale up quickly during failover) and near-zero RPO when paired with Aurora Global Database (replication lag typically under 1 second). Multi-site active/active would also work but costs significantly more. Pilot light has higher RTO (tens of minutes). Backup & Restore has hours-level RTO, which is too slow.

---

### Q687. A company wants to migrate from gp2 EBS volumes to improve performance for its database workload on Amazon EC2. The database requires 10,000 IOPS but only 200 GiB of storage. What is the MOST cost-effective EBS volume type? `Storage`

**Correct Answer:** A. Use gp3 EBS volumes. Configure the volume with 200 GiB storage and 10,000 IOPS.

**Simple Explanation:**
> gp3 volumes let you provision IOPS and throughput independently of storage size. You can get up to 16,000 IOPS on any gp3 volume regardless of size. With gp2, you would need a 3,334 GiB volume to achieve 10,000 IOPS (3 IOPS per GiB), paying for unnecessary storage. gp3 is the default mental model for the SAA-C03 exam — it decouples IOPS from storage size, saving costs. io2 Block Express would also work but costs more.

---

### Q688. A company runs a web application and needs to ensure that EC2 instance metadata cannot be accessed by potential attackers through SSRF (Server-Side Request Forgery) vulnerabilities. Which solution provides the BEST protection? **[IMPORTANT]** `Security`

**Correct Answer:** B. Require the use of Instance Metadata Service Version 2 (IMDSv2) on all EC2 instances.

**Simple Explanation:**
> IMDSv2 requires a session token (obtained via PUT request) before metadata can be accessed. This prevents SSRF attacks because the attacker's forged GET requests won't include the required token. IMDSv1 uses simple GET requests to 169.254.169.254 which are easily exploitable via SSRF. Always use IMDSv2 — this is a frequently tested security topic on the SAA-C03 exam.

---

### Q689. A company wants to provide secure access to internal corporate applications for remote employees without using a traditional VPN. The solution should verify both user identity and device security posture before granting access. Which AWS service meets these requirements? `Security`

**Correct Answer:** A. Use AWS Verified Access to provide zero-trust access to corporate applications.

**Simple Explanation:**
> AWS Verified Access provides secure access to corporate applications without requiring a VPN. It uses zero-trust principles — each access request is evaluated based on the user's identity (via IAM Identity Center, Okta, etc.) and device security posture (via CrowdStrike, Jamf, etc.). This is more secure and easier to manage than traditional VPNs. Client VPN still requires VPN software. PrivateLink is for service-to-service connectivity.

---

### Q690. A company is running an application on Amazon EC2 instances that needs to access Amazon S3. The company's security policy requires that no traffic should traverse the public internet. The company also uses Amazon DynamoDB and Amazon SQS. Which solution ensures private connectivity to ALL three services with the LEAST operational overhead? **[IMPORTANT]** `Security`

**Correct Answer:** C. Create gateway VPC endpoints for S3 and DynamoDB (free, route-table based). Create an interface VPC endpoint (AWS PrivateLink) for SQS.

**Simple Explanation:**
> Gateway VPC endpoints are available for S3 and DynamoDB — they are FREE (no hourly or data charges) and work via route table entries. Always use gateway endpoints when available. Interface VPC endpoints (PrivateLink) are needed for SQS and most other AWS services — they use ENIs in your subnet and have hourly + per-GB charges. Key exam rule: for S3 and DynamoDB, always pick gateway endpoints; for everything else, use interface endpoints (PrivateLink).

---

### Q691. A company has a multi-account AWS environment managed by AWS Organizations. The security team wants to ensure that NO user in ANY member account can disable AWS CloudTrail logging. Which solution meets this requirement? **[IMPORTANT]** `Security`

**Correct Answer:** A. Create a Service Control Policy (SCP) that denies the cloudtrail:StopLogging and cloudtrail:DeleteTrail actions. Attach the SCP to the root organizational unit.

**Simple Explanation:**
> Service Control Policies (SCPs) in AWS Organizations set permission guardrails across all accounts. By attaching a deny SCP to the root OU, you prevent ALL users (including account admins) in ALL member accounts from stopping or deleting CloudTrail. SCPs do not affect the management account. IAM policies alone can be changed by account admins. AWS Config can detect but not prevent the action.

---

### Q692. A company has a data lake in Amazon S3. Different teams need to access different subsets of the data based on their department. The company wants centralized, fine-grained access control over the data lake without managing complex S3 bucket policies for each team. Which solution meets these requirements with the LEAST operational overhead? `Security`

**Correct Answer:** B. Use AWS Lake Formation to define fine-grained permissions at the database, table, and column level for each team.

**Simple Explanation:**
> AWS Lake Formation provides centralized, fine-grained access control for data lakes on S3. You can grant permissions at the database, table, column, and even row level — much simpler than managing individual S3 bucket policies and IAM policies. Lake Formation integrates with AWS Glue, Athena, Redshift Spectrum, and EMR. It replaces the need for complex cross-account S3 policies with a simple grant/revoke model.

---

### Q693. A company is deploying a new application that requires a relational database. The workload is unpredictable — heavy during business hours but nearly idle at night and on weekends. The company wants to minimize costs while ensuring the database can handle peak demand. Which solution is MOST cost-effective? `Database`

**Correct Answer:** C. Use Amazon Aurora Serverless v2 with a minimum of 0.5 ACU and a maximum that handles peak demand.

**Simple Explanation:**
> Aurora Serverless v2 automatically scales compute capacity between a minimum and maximum ACU (Aurora Capacity Unit) in increments of 0.5 ACU. You only pay for the capacity you use. It scales in seconds with no disruption. This is ideal for unpredictable workloads — it scales down during idle periods (nights/weekends) to save costs and scales up instantly for peaks. Provisioned RDS/Aurora would require paying for peak capacity 24/7. Aurora Serverless v1 is deprecated in favor of v2.

---

### Q694. A company uses Amazon S3 to store customer data. Before delivering objects to applications, the company needs to redact personally identifiable information (PII) from the data. The company wants to avoid creating and maintaining duplicate copies of the data. Which solution meets these requirements with the LEAST operational overhead? `Storage`

**Correct Answer:** A. Use S3 Object Lambda to transform the data and redact PII in real time as objects are retrieved.

**Simple Explanation:**
> S3 Object Lambda lets you add your own code (via Lambda functions) to S3 GET requests to modify and process data as it is returned to an application. You can redact PII, resize images, convert formats, etc. — all without storing additional copies of the data. You create an S3 Object Lambda Access Point, and applications use it instead of the original bucket. This avoids duplicating data and keeps a single source of truth.

---

### Q695. A company hosts a web application behind an Application Load Balancer. The application is experiencing bot attacks and credential stuffing attempts. The company wants to block these automated attacks while allowing legitimate users to access the application. Which solution provides the MOST comprehensive protection? `Security`

**Correct Answer:** B. Enable AWS WAF on the ALB. Configure AWS WAF Bot Control to detect and block automated bot traffic. Add rate-based rules to limit requests from individual IP addresses.

**Simple Explanation:**
> AWS WAF Bot Control is a managed rule group that identifies and manages bot traffic (scrapers, crawlers, credential stuffers). Combined with rate-based rules (which automatically block IPs exceeding a threshold), this provides comprehensive protection against automated attacks. Shield Advanced protects against DDoS but not credential stuffing. CloudFront alone doesn't detect bots. Security groups can't do application-layer filtering.

---

### Q696. A company needs to replicate its Amazon RDS for PostgreSQL database to another AWS Region for disaster recovery. The company requires the replica to be available for read queries to serve users in that Region. The RPO must be under 1 second. Which solution meets these requirements? `Database`

**Correct Answer:** A. Use Amazon Aurora PostgreSQL with Aurora Global Database. Create a secondary cluster in the target Region.

**Simple Explanation:**
> Aurora Global Database replicates data across Regions with typical replication lag of under 1 second (RPO < 1 second). The secondary Region cluster supports read queries, serving local users with low latency. Failover to the secondary Region can be done in under 1 minute. Standard RDS cross-Region read replicas have higher replication lag (seconds to minutes). S3 cross-region replication is for objects, not databases.

---

### Q697. A company runs a containerized application on Amazon ECS. The application traffic varies significantly throughout the day. The company wants to minimize costs and eliminate the need to manage servers or capacity planning for the containers. Which solution meets these requirements? `Compute`

**Correct Answer:** B. Use Amazon ECS with AWS Fargate launch type. Configure ECS Service Auto Scaling based on CPU/memory utilization.

**Simple Explanation:**
> Fargate is a serverless compute engine for containers — you don't manage EC2 instances, and you pay only for the vCPU and memory your containers actually use. Combined with ECS Service Auto Scaling, the number of tasks scales up/down based on demand. EC2 launch type requires you to manage and scale the underlying instances. Lambda is for short-lived functions, not long-running containers.

---

### Q698. A company has an on-premises application that generates 50 TB of data daily. The company needs to transfer this data to Amazon S3 every day with minimal latency. The company already has a 10 Gbps AWS Direct Connect connection. Which solution meets these requirements? `Migration`

**Correct Answer:** C. Use AWS DataSync with the existing Direct Connect connection to transfer data daily to Amazon S3.

**Simple Explanation:**
> AWS DataSync can transfer data at speeds up to 10x faster than open-source tools by using a purpose-built network protocol. It works over Direct Connect for fast, reliable, scheduled transfers. 50 TB per day over a 10 Gbps connection is feasible (~11 hours). Snowball is for offline/one-time bulk migration and takes days for shipping. S3 Transfer Acceleration is for internet uploads, not Direct Connect.

---

### Q699. A company wants to encrypt all Amazon EBS volumes automatically at creation without requiring developers to remember to enable encryption. Which solution enforces this requirement across the entire AWS account with the LEAST effort? **[IMPORTANT]** `Security`

**Correct Answer:** A. Enable EBS encryption by default in the EC2 settings for the AWS account in each Region.

**Simple Explanation:**
> When you enable "EBS encryption by default" in the EC2 console settings, all new EBS volumes and snapshots are automatically encrypted — developers don't need to do anything. This is a per-Region, per-account setting. It uses the default AWS managed KMS key (aws/ebs) unless you specify a customer managed key. SCPs can prevent disabling this setting. AWS Config can detect unencrypted volumes but cannot enforce encryption at creation.

---

### Q700. A company has multiple VPCs across three AWS Regions. Applications in each VPC need to communicate with applications in every other VPC. The company also has an on-premises data center connected via AWS Direct Connect. The company wants to simplify network management. Which solution meets these requirements with the LEAST operational overhead? `Networking`

**Correct Answer:** D. Create a transit gateway in each Region. Set up transit gateway peering between Regions. Attach each VPC to its regional transit gateway. Connect Direct Connect to one transit gateway using a transit VIF.

**Simple Explanation:**
> Transit Gateway acts as a central hub — all VPCs in a Region attach to it, and transit gateway peering connects across Regions. This creates a hub-and-spoke model that scales far better than full-mesh VPC peering. Direct Connect connects via a transit virtual interface (transit VIF) to one transit gateway, and routing propagates to all connected VPCs. VPC peering requires a separate connection for each VPC pair (N^2 complexity).

---

### Q701. A company is running a critical web application. The company needs to detect threats such as unauthorized API calls, cryptocurrency mining on EC2 instances, and compromised credentials. The solution should be enabled with minimal configuration. Which AWS service meets these requirements? `Security`

**Correct Answer:** B. Enable Amazon GuardDuty in the account.

**Simple Explanation:**
> Amazon GuardDuty is a threat detection service that continuously monitors for malicious activity and unauthorized behavior. It analyzes CloudTrail logs, VPC Flow Logs, and DNS logs using machine learning to detect threats like compromised instances, credential theft, cryptocurrency mining, and unusual API calls. It requires no infrastructure — just enable it with one click. Inspector is for vulnerability scanning. Macie is for S3 data classification. Security Hub aggregates findings but doesn't generate them.

---

### Q702. A company stores sensitive financial documents in Amazon S3. Compliance requires that the documents cannot be deleted or modified for 7 years, even by the root user. Which solution meets this requirement? **[IMPORTANT]** `Storage`

**Correct Answer:** C. Enable S3 Object Lock in compliance mode on the bucket. Set a retention period of 7 years.

**Simple Explanation:**
> S3 Object Lock in **compliance mode** prevents ANY user (including the root user) from deleting or overwriting objects until the retention period expires. This is stricter than governance mode, which allows users with special permissions to override the lock. Versioning must be enabled for Object Lock. This is the only way to achieve true WORM (Write Once Read Many) compliance on S3. Bucket policies can be changed by admins, so they alone are insufficient.

---

### Q703. A company runs an application that processes images uploaded by users. The processing takes 5-10 minutes per image. During peak hours, thousands of images are uploaded simultaneously. The company wants a serverless, cost-effective solution that can handle the burst without losing any uploads. Which architecture meets these requirements? `Compute`

**Correct Answer:** B. Configure S3 event notifications to send messages to an Amazon SQS queue when images are uploaded. Use an AWS Lambda function that polls the SQS queue to process images. Configure Lambda reserved concurrency to control throughput.

**Simple Explanation:**
> SQS acts as a buffer between the upload spike and processing, ensuring no images are lost even during massive bursts. Lambda can poll SQS and process messages. However, Lambda has a 15-minute max timeout, so 5-10 minute processing fits. SQS retains messages for up to 14 days if processing is delayed. Reserved concurrency prevents Lambda from being overwhelmed. This pattern (S3 -> SQS -> Lambda) is a core serverless pattern tested heavily on the exam.

---

### Q704. A company has multiple AWS accounts and wants to share Amazon VPC subnets across accounts so that EC2 instances in different accounts can be launched in the same VPC. Which solution meets this requirement? `Networking`

**Correct Answer:** A. Use AWS Resource Access Manager (RAM) to share VPC subnets across accounts within the same AWS Organization.

**Simple Explanation:**
> AWS Resource Access Manager (RAM) allows you to share VPC subnets with other AWS accounts in the same Organization. This is called VPC sharing — resources in different accounts can be launched in the same shared subnet, communicating via private IPs without VPC peering or transit gateways. The VPC owner creates and manages the VPC, while participants launch resources in shared subnets. This reduces VPC sprawl and simplifies networking.

---

### Q705. A company has a legacy application that writes log data to a local filesystem. The company wants to move this application to AWS without modifying the application code. Multiple EC2 instances need to access the same log data simultaneously. Which storage solution meets these requirements? `Storage`

**Correct Answer:** B. Create an Amazon EFS file system. Mount the EFS file system on all EC2 instances.

**Simple Explanation:**
> Amazon EFS provides a POSIX-compliant shared file system that multiple EC2 instances can mount simultaneously across Availability Zones. Since the application writes to a local filesystem path, EFS can be mounted at that path with no code changes. EBS volumes can only be attached to one instance at a time (except io1/io2 Multi-Attach, which is block-level, not filesystem). S3 requires API calls, not filesystem access. FSx for Lustre is for high-performance computing, not general logging.

---

### Q706. A company wants to automatically detect and alert when any Amazon S3 bucket is made publicly accessible. The solution should work across all AWS accounts in the organization. Which approach meets these requirements with the LEAST operational overhead? `Security`

**Correct Answer:** C. Enable AWS Security Hub across the organization. Use the AWS Foundational Security Best Practices standard, which includes checks for public S3 buckets.

**Simple Explanation:**
> AWS Security Hub provides automated, continuous security checks against best practices across your organization. The AWS Foundational Security Best Practices standard includes specific controls that detect public S3 buckets. Security Hub aggregates findings from multiple accounts and can trigger SNS notifications or EventBridge rules for alerting. AWS Config rules can also detect this, but Security Hub provides a centralized dashboard and cross-account aggregation with less setup.

---

### Q707. A company runs a web application that serves users in North America and Europe. During peak hours, European users experience high latency because the application is hosted only in us-east-1. The company wants to reduce latency for European users without deploying a full application stack in Europe. Which solution meets these requirements MOST cost-effectively? `Networking`

**Correct Answer:** B. Create an Amazon CloudFront distribution with the ALB in us-east-1 as the origin. CloudFront will cache content at edge locations near European users.

**Simple Explanation:**
> CloudFront caches content at 400+ edge locations worldwide. European users will receive cached static content from nearby edge locations instead of making round trips to us-east-1. For dynamic content, CloudFront uses optimized network paths (AWS backbone) which is still faster than public internet. This is the most cost-effective approach — no need to deploy infrastructure in Europe. Global Accelerator improves latency for TCP/UDP traffic but doesn't cache. Deploying in eu-west-1 works but costs more.

---

### Q708. A company is building a real-time data analytics pipeline. Millions of events per second from IoT devices need to be ingested, processed in real-time, and stored for analysis. Which architecture handles this MOST effectively? `Analytics`

**Correct Answer:** A. Use Amazon Kinesis Data Streams to ingest events. Use an AWS Lambda function (or Kinesis Data Analytics) for real-time processing. Use Amazon Kinesis Data Firehose to deliver processed data to Amazon S3 for analysis with Amazon Athena.

**Simple Explanation:**
> Kinesis Data Streams can ingest millions of events per second with low latency. Lambda or Kinesis Data Analytics can process events in real time. Kinesis Data Firehose automatically batches and delivers data to S3 (or Redshift, OpenSearch). Athena then provides serverless SQL queries on the stored data. SQS is for message queuing, not real-time streaming. DynamoDB Streams are for database change events, not IoT ingestion.

---

### Q709. A company runs a web application with an Amazon RDS for MySQL database. The application occasionally runs complex reporting queries that degrade performance for regular users. The company wants to isolate the reporting workload without modifying the application. Which solution meets these requirements with the LEAST operational overhead? `Database`

**Correct Answer:** B. Create an RDS read replica. Direct the reporting queries to the read replica endpoint.

**Simple Explanation:**
> RDS read replicas are asynchronous copies of the primary database that can handle read queries. By directing reporting queries to the read replica, you offload the heavy reporting workload from the primary database, preventing performance degradation for regular users. Read replicas can be created with a few clicks and use the same engine version. Redshift is for data warehousing and requires ETL. ElastiCache caches results but doesn't run complex SQL queries.

---

### Q710. A company has a three-tier application (web, app, database) running in a single VPC. The security team wants to ensure that the database tier can ONLY receive traffic from the application tier, and the application tier can ONLY receive traffic from the web tier. Which solution implements this requirement? `Security`

**Correct Answer:** A. Configure security groups for each tier. The database security group allows inbound traffic only from the application tier security group. The application security group allows inbound traffic only from the web tier security group.

**Simple Explanation:**
> Security groups can reference other security groups as the source in inbound rules. By configuring the database SG to allow inbound traffic only from the app-tier SG (not by IP), and the app-tier SG to allow only from the web-tier SG, you create strict tier isolation. This is more maintainable than IP-based rules because instances can be added/removed from tiers without updating rules. NACLs are stateless and harder to manage for this purpose.

---

### Q711. A company wants to run a cost-effective development/test environment. The workloads are interruptible and can be restarted if stopped. The company needs the lowest possible cost. Which EC2 pricing option meets these requirements? `Compute`

**Correct Answer:** A. Use Spot Instances for the development and test workloads.

**Simple Explanation:**
> Spot Instances offer up to 90% discount compared to On-Demand pricing. They are ideal for workloads that are fault-tolerant and can handle interruptions — such as development, testing, batch processing, and CI/CD pipelines. AWS can reclaim Spot Instances with a 2-minute warning when capacity is needed. Reserved Instances require 1-3 year commitment and are for steady-state production. On-Demand is the most expensive option.

---

### Q712. A company stores application logs in Amazon S3. The company needs to query these logs using standard SQL but does NOT want to load the data into a database. Queries are ad-hoc and run infrequently. Which solution is MOST cost-effective? `Analytics`

**Correct Answer:** B. Use Amazon Athena to query the logs directly in S3 using SQL.

**Simple Explanation:**
> Amazon Athena is a serverless query service that lets you analyze data directly in S3 using standard SQL. You pay only per query based on the amount of data scanned — no infrastructure to manage. For infrequent, ad-hoc queries, this is far cheaper than running Redshift (requires provisioned cluster) or EMR (requires cluster management). Tip: store logs in columnar format (Parquet/ORC) and partition by date to reduce scan costs.

---

### Q713. A company is designing an architecture that must remain operational even if an entire AWS Region becomes unavailable. The application uses an Amazon Aurora MySQL database. Which solution provides multi-Region database resilience? `Database`

**Correct Answer:** C. Use Amazon Aurora Global Database with a primary cluster in one Region and secondary clusters in other Regions.

**Simple Explanation:**
> Aurora Global Database spans multiple Regions with replication lag typically under 1 second. If the primary Region fails, you can promote a secondary Region cluster to primary in under 1 minute. Secondary clusters serve read traffic locally, reducing latency for users in those Regions. Cross-Region RDS read replicas have higher lag and longer promotion time. DynamoDB Global Tables solve the same problem for NoSQL workloads.

---

### Q714. A company wants to reduce costs for its Amazon S3 storage. The data access patterns show that objects are accessed frequently for 30 days, occasionally for the next 90 days, and rarely after that but must be retained for 7 years. Which S3 lifecycle configuration is MOST cost-effective? **[IMPORTANT]** `Storage`

**Correct Answer:** B. Keep objects in S3 Standard for 30 days, transition to S3 Standard-IA after 30 days, transition to S3 Glacier Flexible Retrieval after 120 days, and transition to S3 Glacier Deep Archive after 365 days.

**Simple Explanation:**
> This lifecycle policy matches storage class to access pattern: Standard for active access (30 days), Standard-IA for occasional access (30-120 days, cheaper storage but retrieval fee), Glacier Flexible for rare access (120-365 days, minutes-to-hours retrieval), and Deep Archive for long-term retention (7 years, cheapest at ~$1/TB/month, 12-48 hour retrieval). Each transition reduces cost as access frequency drops. Intelligent-Tiering could also work but has monitoring fees.

---

### Q715. A company has deployed an application across two Availability Zones behind an Application Load Balancer. The company wants to ensure the application stays available even if one AZ fails. Currently, each AZ runs 3 EC2 instances (6 total), and the minimum required to handle traffic is 4 instances. How should the Auto Scaling group be configured? `Compute`

**Correct Answer:** C. Configure the Auto Scaling group with a minimum of 8 instances (4 per AZ) across both Availability Zones.

**Simple Explanation:**
> To survive an AZ failure, each AZ must independently handle the full workload. If you need a minimum of 4 instances, you must run 4 in each AZ (8 total). If AZ-A fails, the 4 instances in AZ-B handle all traffic. Running only 3 per AZ (6 total) means that if one AZ fails, you only have 3 instances — below the required 4. This "N+1 AZ" principle is a key high availability concept on the exam.

---

### Q716. A company wants to centralize security findings from Amazon GuardDuty, Amazon Inspector, AWS Config, and third-party security tools into a single dashboard. Which AWS service should the company use? `Security`

**Correct Answer:** A. Use AWS Security Hub to aggregate and prioritize security findings from multiple services.

**Simple Explanation:**
> AWS Security Hub is a centralized security dashboard that aggregates findings from GuardDuty (threat detection), Inspector (vulnerability scanning), Config (compliance), Macie (data classification), Firewall Manager, and third-party tools. It normalizes findings into a standard format (AWS Security Finding Format), provides compliance checks against CIS Benchmarks, and supports automated remediation via EventBridge. It works across accounts via AWS Organizations.

---

### Q717. A company runs an application on EC2 instances that communicate with Amazon DynamoDB. The security team requires that all communication between EC2 and DynamoDB stays within the AWS network and does NOT traverse the public internet. What is the MOST cost-effective solution? `Networking`

**Correct Answer:** A. Create a gateway VPC endpoint for DynamoDB.

**Simple Explanation:**
> Gateway VPC endpoints for DynamoDB (and S3) are free — no hourly charge and no data processing charge. They route traffic to DynamoDB through the AWS private network via route table entries. Interface endpoints (PrivateLink) also keep traffic private but have hourly and per-GB charges. NAT Gateways route traffic through the internet, which violates the security requirement. Always prefer gateway endpoints when available (S3 and DynamoDB) — they're free and private.

---

### Q718. A company needs to run a batch processing job every night that takes 2-4 hours to complete. The job processes data from Amazon S3 and writes results back to S3. The job can be restarted if interrupted. Which compute solution is MOST cost-effective? `Compute`

**Correct Answer:** D. Use AWS Batch with Spot Instances to run the batch job.

**Simple Explanation:**
> AWS Batch automatically provisions the optimal compute resources (EC2 or Fargate) for batch jobs. Using Spot Instances provides up to 90% cost savings. Since the job can be restarted if interrupted, Spot interruptions are acceptable. AWS Batch handles job scheduling, retry logic, and resource management. Lambda has a 15-minute timeout limit which is insufficient for 2-4 hour jobs. On-Demand instances would cost significantly more.

---

### Q719. A company has an application that needs to send the same message to multiple SQS queues and Lambda functions simultaneously when an event occurs. Which architecture provides this fan-out capability? `Integration`

**Correct Answer:** B. Publish the event to an Amazon SNS topic. Subscribe the SQS queues and Lambda functions to the SNS topic.

**Simple Explanation:**
> SNS provides publish/subscribe (pub/sub) messaging for fan-out patterns. When a message is published to an SNS topic, it is delivered to ALL subscribers simultaneously — multiple SQS queues, Lambda functions, HTTP endpoints, email, etc. This is the standard fan-out pattern in AWS. Sending to each queue individually from the application creates tight coupling and doesn't scale. EventBridge could also fan-out but SNS is simpler for this use case.

---

### Q720. A company wants to optimize its AWS spending. The company has steady-state workloads running 24/7 and knows the instance types needed for the next 3 years. However, the company also wants flexibility to change instance families in the future. Which pricing model provides the BEST cost savings with this flexibility? `Compute`

**Correct Answer:** C. Purchase a Compute Savings Plan with a 3-year term.

**Simple Explanation:**
> Compute Savings Plans provide up to 66% discount (similar to Reserved Instances) but with flexibility to change instance family, size, OS, tenancy, and even Region. Standard Reserved Instances are locked to a specific instance type and Region. Convertible Reserved Instances allow changes but with less discount. Compute Savings Plans apply to EC2, Fargate, and Lambda usage. For known steady-state workloads where you want flexibility, Compute Savings Plans are the best choice.

---

### Q721. A company needs to detect sensitive data such as credit card numbers, social security numbers, and personal health information stored in Amazon S3 buckets. The solution must be automated and require minimal setup. Which service meets these requirements? `Security`

**Correct Answer:** A. Enable Amazon Macie and configure it to scan the S3 buckets for sensitive data.

**Simple Explanation:**
> Amazon Macie uses machine learning and pattern matching to automatically discover and classify sensitive data (PII, PHI, financial data) stored in S3. It provides alerts when sensitive data is found and gives visibility into the data security posture of your S3 environment. It requires minimal setup — enable it and point it at your buckets. AWS Config checks configuration, not data content. Athena requires manual queries. GuardDuty monitors for threats, not data classification.

---

### Q722. A company has a hybrid architecture with an on-premises data center connected to AWS via Direct Connect. The Direct Connect link occasionally fails. The company needs a reliable backup connection that can be set up quickly. Which solution provides the MOST cost-effective backup? **[IMPORTANT]** `Networking`

**Correct Answer:** B. Create a Site-to-Site VPN connection as a backup for the Direct Connect link. Configure BGP to automatically failover to the VPN when Direct Connect is unavailable.

**Simple Explanation:**
> A Site-to-Site VPN over the public internet provides a cost-effective backup for Direct Connect. Using BGP (Border Gateway Protocol), traffic automatically fails over to the VPN when Direct Connect goes down, and switches back when it recovers. A second Direct Connect at a different location provides higher bandwidth but costs significantly more. The VPN backup provides adequate bandwidth for most failover scenarios and can be set up in minutes.

---

### Q723. A company runs a microservices application where Service A depends on Service B. When Service B is overloaded, Service A also fails. The company wants to prevent Service B failures from cascading to Service A. Which architecture pattern solves this problem? `Integration`

**Correct Answer:** C. Place an Amazon SQS queue between Service A and Service B. Service A sends requests to the queue, and Service B processes them at its own pace.

**Simple Explanation:**
> SQS acts as a buffer (decoupling layer) between services. When Service B is overloaded, messages queue up in SQS instead of causing Service A to fail. Service B processes messages at its own rate and catches up when load decreases. This prevents cascade failures and is the core "decoupling" pattern tested repeatedly on the SAA-C03 exam. Circuit breakers are an application-level pattern. Direct retries still create coupling.

---

### Q724. A company needs to host a static website (HTML, CSS, JavaScript) that serves users globally. The solution must be highly available, scalable, and cost-effective with NO servers to manage. Which architecture meets these requirements? `Storage`

**Correct Answer:** A. Host the website in an Amazon S3 bucket with static website hosting enabled. Use Amazon CloudFront as a CDN in front of the S3 bucket.

**Simple Explanation:**
> S3 static website hosting + CloudFront is the standard serverless pattern for static websites. S3 stores the files durably (11 nines), CloudFront caches them at 400+ global edge locations for low latency, and both are fully managed with no servers. This scales to millions of requests and costs only pennies. Use an Origin Access Control (OAC) to ensure users access S3 only through CloudFront. EC2 instances add unnecessary cost and management overhead.

---

### Q725. A company is using Amazon RDS for PostgreSQL. The database handles both OLTP (transactional) and OLAP (analytical) queries. The OLAP queries are causing performance issues for OLTP workloads. The company wants to separate these workloads. Which solution is MOST appropriate? `Database`

**Correct Answer:** D. Create an RDS read replica for OLAP queries. Alternatively, use AWS DMS to replicate data to Amazon Redshift for analytical workloads.

**Simple Explanation:**
> For simple OLAP queries, a read replica offloads reads from the primary. For complex analytics, Amazon Redshift (columnar data warehouse) is designed for OLAP — it runs analytical queries orders of magnitude faster than PostgreSQL. Use DMS for continuous replication from RDS to Redshift. This separates OLTP and OLAP workloads completely. ElastiCache helps with repeated queries but not complex analytics. Scaling up the RDS instance costs more and doesn't solve the fundamental workload conflict.

---

### Q726. A company is running an application in us-east-1 that serves API requests. The company is expanding to serve users in Asia and Europe. Users in those regions experience high latency. The application cannot be deployed to multiple regions due to data residency requirements. Which solution reduces latency for international users? `Networking`

**Correct Answer:** B. Use AWS Global Accelerator to route traffic to the application in us-east-1 through the AWS global network.

**Simple Explanation:**
> AWS Global Accelerator provides static anycast IP addresses that route traffic to the nearest AWS edge location, then through the AWS global backbone network to your application. This is significantly faster than routing over the public internet, especially for international traffic. Unlike CloudFront, Global Accelerator works for non-cacheable, dynamic API requests and maintains TCP/UDP connections. It reduces latency by 60% or more for long-distance traffic.

---

### Q727. A company runs Amazon EC2 instances with sensitive data. The security team wants to ensure that any unauthorized access attempts to EC2 instances from the internet are detected and logged. Which combination provides the BEST visibility? (Choose two.) `Security`

**Correct Answer:** B. Enable VPC Flow Logs to capture network traffic metadata. D. Enable Amazon GuardDuty to detect threats from analyzing flow logs, DNS logs, and CloudTrail events.

**Simple Explanation:**
> VPC Flow Logs capture metadata (source IP, destination IP, port, protocol, action) for all network traffic to/from network interfaces in your VPC. This provides raw visibility into all connection attempts. GuardDuty analyzes these flow logs (plus DNS and CloudTrail logs) using machine learning to detect anomalies like port scanning, brute force attacks, or compromised instances communicating with known malicious IPs. Together, they provide comprehensive detection and logging.

---

### Q728. A company stores data in Amazon DynamoDB. The application requires single-digit millisecond latency for read operations, but during peak hours, the read latency increases to 50+ milliseconds due to high traffic. Which solution reduces read latency with the LEAST changes to the application? `Database`

**Correct Answer:** A. Add Amazon DynamoDB Accelerator (DAX) as an in-memory cache in front of DynamoDB.

**Simple Explanation:**
> DAX is a fully managed, in-memory cache specifically designed for DynamoDB. It reduces read latency from single-digit milliseconds to microseconds, even under heavy load. DAX is API-compatible with DynamoDB — you just change the endpoint in your application (minimal code change). ElastiCache (Redis/Memcached) requires more application changes to implement caching logic. Read capacity auto-scaling helps with throughput but doesn't improve latency as dramatically as caching.

---

### Q729. A company needs to run a machine learning inference endpoint that must be available 24/7 with auto-scaling based on traffic. The company does NOT want to manage infrastructure. Which AWS service meets these requirements? `Compute`

**Correct Answer:** C. Use Amazon SageMaker real-time inference endpoints with auto-scaling configured.

**Simple Explanation:**
> SageMaker real-time inference endpoints provide fully managed ML model hosting with built-in auto-scaling. You deploy a trained model, and SageMaker manages the infrastructure, scaling instances up/down based on traffic. There's no need to manage EC2 instances, containers, or load balancers. For cost optimization, SageMaker also offers serverless inference (for intermittent traffic) and batch transform (for offline predictions).

---

### Q730. A company has a mobile application that needs to authenticate users and provide temporary AWS credentials for accessing S3 and DynamoDB. The company wants to support social identity providers (Google, Facebook, Apple) and its own user directory. Which solution meets these requirements with the LEAST operational overhead? **[IMPORTANT]** `Security`

**Correct Answer:** A. Use Amazon Cognito user pools for authentication and Amazon Cognito identity pools (federated identities) for providing temporary AWS credentials.

**Simple Explanation:**
> Amazon Cognito user pools handle user sign-up/sign-in and integrate with social identity providers (Google, Facebook, Apple) and SAML/OIDC providers. Cognito identity pools exchange user pool tokens for temporary AWS credentials (via STS) scoped to specific IAM roles. This two-step flow (authenticate with user pool, authorize with identity pool) is the standard mobile auth pattern. Building your own auth with IAM federation or custom STS calls is significantly more complex.

---

### Q731. A company hosts a critical production database on Amazon RDS for MySQL. The company needs to ensure the database is available even during hardware failures in an Availability Zone. The company also needs automated backups with point-in-time recovery. Which configuration meets these requirements? `Database`

**Correct Answer:** B. Enable RDS Multi-AZ deployment and enable automated backups with a sufficient retention period.

**Simple Explanation:**
> RDS Multi-AZ maintains a synchronous standby replica in a different AZ. If the primary fails (hardware, AZ outage, etc.), RDS automatically fails over to the standby with minimal downtime (typically 60-120 seconds). Automated backups with point-in-time recovery let you restore to any second within the retention period (up to 35 days). Multi-AZ is for high availability (not read scaling — that's read replicas). This is one of the most fundamental RDS concepts on the exam.

---

### Q732. A company is building a data pipeline that receives CSV files in S3, transforms them to Parquet format, catalogs the data, and makes it available for SQL queries. Which serverless solution handles this end-to-end? `Analytics`

**Correct Answer:** A. Use AWS Glue crawlers to discover the schema and populate the Data Catalog. Use AWS Glue ETL jobs to transform CSV to Parquet. Use Amazon Athena to run SQL queries on the Parquet data.

**Simple Explanation:**
> This is the standard serverless analytics pipeline: Glue crawlers automatically detect schema and create table definitions in the Glue Data Catalog. Glue ETL jobs transform CSV to Parquet (columnar format, 3-5x faster queries, 80% cheaper Athena scans). Athena queries the Parquet data directly in S3 using SQL. All three services are serverless — no infrastructure to manage. EMR could do this but requires cluster management.

---

### Q733. A company has a compliance requirement to retain AWS CloudTrail logs for 10 years. The logs must be tamper-proof and immutable. Which solution meets these requirements with the LEAST cost? `Monitoring`

**Correct Answer:** C. Configure CloudTrail to deliver logs to an S3 bucket. Enable S3 Object Lock in compliance mode with a 10-year retention. Use S3 Lifecycle policies to transition logs to S3 Glacier Deep Archive after 90 days.

**Simple Explanation:**
> CloudTrail delivers logs to S3. Object Lock in compliance mode makes them immutable for 10 years (no one can delete them, not even root). Transitioning to Glacier Deep Archive after 90 days reduces storage cost to ~$1/TB/month, making 10-year retention affordable. CloudTrail log file integrity validation ensures logs haven't been tampered with. CloudWatch Logs retention is more expensive for long-term storage and doesn't provide the same immutability guarantees.

---

### Q734. A company has an application that writes data to Amazon DynamoDB. The company also needs to trigger real-time notifications whenever a new item is inserted into the table. Which solution meets these requirements with the LEAST operational overhead? `Integration`

**Correct Answer:** A. Enable DynamoDB Streams on the table. Create an AWS Lambda function triggered by the stream to send notifications via Amazon SNS.

**Simple Explanation:**
> DynamoDB Streams captures a time-ordered sequence of item-level changes (insert, update, delete) in a DynamoDB table. Lambda can automatically poll the stream and process each change. When a new item is inserted, Lambda publishes a notification to SNS which delivers to subscribers (email, SMS, HTTP, SQS, etc.). This is a fully serverless, real-time event pipeline with no infrastructure to manage. Polling DynamoDB directly is inefficient and costly.

---

### Q735. A company wants to migrate on-premises VMware virtual machines to AWS with minimal downtime. The company needs continuous replication of servers to AWS and the ability to launch test instances before the final cutover. Which AWS service meets these requirements? `Migration`

**Correct Answer:** A. Use AWS Elastic Disaster Recovery (AWS DRS) to continuously replicate on-premises servers to AWS and perform non-disruptive testing before cutover.

**Simple Explanation:**
> AWS Elastic Disaster Recovery (formerly CloudEndure) provides continuous block-level replication of servers to AWS. It maintains staging resources in your AWS account at low cost, and you can launch fully provisioned test or recovery instances in minutes. During migration, it supports non-disruptive testing (launch test instances without affecting replication) and quick cutover with minimal downtime. This is the go-to service for both DR and server migration with minimal downtime.

---

### Q736. A company is running multiple EC2 instances of different types and sizes. The operations team wants to identify which instances are over-provisioned or under-provisioned to optimize costs and performance. Which AWS service provides these recommendations automatically? `Compute`

**Correct Answer:** B. Use AWS Compute Optimizer to analyze instance utilization and get right-sizing recommendations.

**Simple Explanation:**
> AWS Compute Optimizer uses machine learning to analyze historical utilization metrics (CPU, memory, network, disk) and recommends optimal EC2 instance types and sizes. It identifies over-provisioned instances (wasting money) and under-provisioned instances (risking performance). It also provides recommendations for EBS volumes, Lambda functions, and ECS on Fargate. Cost Explorer also has right-sizing recommendations, but Compute Optimizer provides more detailed, ML-driven analysis.

---

### Q737. A company manages WAF rules, Shield Advanced protections, and security groups across 50 AWS accounts. The security team wants to centrally manage these firewall rules from a single account. Which solution meets these requirements? `Security`

**Correct Answer:** A. Use AWS Firewall Manager to centrally configure and manage WAF rules, Shield Advanced, and security groups across all accounts in the AWS Organization.

**Simple Explanation:**
> AWS Firewall Manager lets you centrally configure and manage firewall rules across accounts and resources in your Organization. It supports AWS WAF rules, Shield Advanced protections, VPC security groups, Network Firewall policies, and Route 53 Resolver DNS Firewall rules. When new accounts or resources are created, Firewall Manager automatically applies the policies. This eliminates the need to configure security in each account individually.

---

### Q738. A company runs a high-performance computing (HPC) workload that processes large datasets stored in Amazon S3. The processing requires a high-throughput, POSIX-compatible file system that is tightly integrated with S3. Which storage solution meets these requirements? `Storage`

**Correct Answer:** C. Use Amazon FSx for Lustre linked to the S3 bucket.

**Simple Explanation:**
> FSx for Lustre is a high-performance file system designed for HPC, machine learning, and media processing workloads. It provides sub-millisecond latencies and throughput of hundreds of GB/s. When linked to an S3 bucket, FSx for Lustre automatically presents S3 objects as files and can write results back to S3. EFS is simpler but slower. FSx for Windows is for Windows workloads. Mounting S3 directly (via S3 Mountpoint) provides limited POSIX compatibility.

---

### Q739. A company wants to ensure that all resources across all AWS accounts in its Organization are tagged correctly for cost allocation. Non-compliant resources should be flagged automatically. Which solution provides this with the LEAST effort? `Monitoring`

**Correct Answer:** B. Use AWS Config rules with the required-tags managed rule across all accounts via AWS Organizations. Use AWS Config aggregator for centralized reporting.

**Simple Explanation:**
> AWS Config managed rule "required-tags" checks whether resources have the required tags. By deploying Config rules across all Organization accounts and using a Config aggregator, you get a centralized view of non-compliant resources. AWS Organizations can deploy Config rules to all member accounts simultaneously. This is automated and requires no custom code. AWS Tag Editor can check tags but doesn't enforce or alert. SCPs cannot enforce tags at creation.

---

### Q740. A company has a REST API that needs to handle user authentication and rate-limiting. The API is built with Lambda functions. The company wants a fully managed solution that handles API versioning, throttling, and caching. Which service should the company use? `Integration`

**Correct Answer:** A. Use Amazon API Gateway to create and manage the REST API with built-in throttling, caching, and authentication support.

**Simple Explanation:**
> API Gateway is a fully managed service for creating, publishing, and securing REST/HTTP/WebSocket APIs. It provides built-in throttling (rate limiting per client with API keys and usage plans), response caching (reduces Lambda invocations), API versioning (stages like dev/prod), and authentication (IAM, Cognito, Lambda authorizers). It integrates directly with Lambda, making it the standard front-door for serverless APIs. ALB can route to Lambda but lacks API management features.

---

---

# MUST-READ CONCEPTS FOR THE EXAM

> Read these concepts carefully. The exam tests your **understanding of architecture patterns**, not just service names. Each section below is something you MUST know before exam day.

---

## Concept 1: S3 Storage Classes (Know This Cold)
...
S3 Standard          → Frequent access, millisecond retrieval
  │                     Cost: ~$0.023/GB. No retrieval fee.
  ▼
S3 Standard-IA       → Infrequent access (min 30 days)
  │                     Cost: ~$0.0125/GB. Has retrieval fee.
  │                     Min object size: 128 KB
  ▼
S3 One Zone-IA       → Same as IA but ONE AZ only (less durable)
  │                     Cost: ~$0.01/GB. Use for reproducible data only.
  ▼
S3 Intelligent-      → Unpredictable access pattern
  Tiering               Auto-moves between tiers. Small monitoring fee.
  │                     Best when you don't know access patterns.
  ▼
S3 Glacier Instant   → Archive with MILLISECOND retrieval
  Retrieval              Cost: ~$0.004/GB. Same speed as Standard.
  │                     Use: medical images, news archives
  ▼
S3 Glacier Flexible  → Archive, retrieval in MINUTES to HOURS
  Retrieval              Cost: ~$0.0036/GB
  │                     Expedited: 1-5 min | Standard: 3-5 hrs | Bulk: 5-12 hrs
  ▼
S3 Glacier Deep      → Cheapest. Retrieval in 12-48 HOURS
  Archive                Cost: ~$0.00099/GB (~$1/TB/month!)
                         Use: compliance, 7-10 year retention
```

**Key exam rules:**
- Standard-IA minimum storage = 30 days (you pay for 30 days even if deleted earlier)
- Glacier minimum = 90 days, Deep Archive minimum = 180 days
- Lifecycle policies can ONLY move objects DOWN (Standard → IA → Glacier), never UP
- S3 Intelligent-Tiering has NO retrieval fees, just a small monitoring fee per object

---

## Concept 2: EBS Volume Types (Very Important for Exam)

| Volume | Type | Max IOPS | Max Throughput | Use Case |
|--------|------|----------|----------------|----------|
| **gp3** | General SSD | 16,000 | 1,000 MB/s | **Default choice.** IOPS and throughput set independently of size |
| **gp2** | General SSD | 16,000 | 250 MB/s | Legacy. IOPS = 3 × GB (burst to 3,000). Larger disk = more IOPS |
| **io2 Block Express** | Provisioned IOPS SSD | 256,000 | 4,000 MB/s | Critical databases, highest performance |
| **io1** | Provisioned IOPS SSD | 64,000 | 1,000 MB/s | Older provisioned IOPS |
| **st1** | Throughput HDD | 500 | 500 MB/s | Big data, log processing, sequential reads |
| **sc1** | Cold HDD | 250 | 250 MB/s | Cheapest. Infrequent access, archival |

**Key exam rules:**
- **gp3 is the answer 90% of the time** — decouples IOPS from size
- gp2 needs 3,334 GB to get 10,000 IOPS (3 IOPS/GB), gp3 just provisions it
- **Only io1/io2 support Multi-Attach** (one volume attached to multiple EC2 in same AZ)
- **HDD (st1/sc1) CANNOT be boot volumes** — only SSD can boot
- EBS is **single-AZ**. Use snapshots to copy across AZs/Regions
- **EBS encryption by default** can be enabled per Region per account

---

## Concept 3: Database Selection Guide

```
Question: "What type of data?"
  │
  ├── Relational (SQL, tables, joins, ACID)?
  │     ├── Need MySQL/PostgreSQL at high performance? → Aurora
  │     ├── Need Oracle/SQL Server/MariaDB?            → RDS
  │     ├── Need serverless relational DB?             → Aurora Serverless v2
  │     └── Need cross-Region replication < 1s lag?    → Aurora Global Database
  │
  ├── Key-Value / NoSQL (flexible schema)?
  │     ├── Millisecond latency at any scale?          → DynamoDB
  │     ├── Need microsecond reads (caching)?          → DynamoDB + DAX
  │     └── Need multi-Region active-active NoSQL?     → DynamoDB Global Tables
  │
  ├── In-Memory / Caching?
  │     ├── Session store, leaderboards, pub/sub?      → ElastiCache Redis
  │     └── Simple key-value cache?                    → ElastiCache Memcached
  │
  ...
  │
  └── Document / JSON?                                 → DocumentDB (MongoDB compat)
```

**Key exam rules:**
- **RDS Multi-AZ** = high availability (automatic failover, NOT for reads)
- **RDS Read Replica** = read scaling (can be cross-Region, NOT automatic failover)
- **Aurora** = 5x faster than MySQL, 3x faster than PostgreSQL, up to 15 read replicas
- **Aurora Serverless v2** = scales to zero, ideal for unpredictable or infrequent workloads
- **RDS Proxy** = connection pooling, great with Lambda (prevents connection exhaustion)
- **DynamoDB on-demand** = no capacity planning; provisioned = cheaper for steady workloads

---

## Concept 4: VPC Networking (Must Understand)
...
│  VPC Endpoint (Gateway)  ←→ S3, DynamoDB (FREE, private)          │
│  VPC Endpoint (Interface)←→ SQS, SNS, KMS, etc (paid, private)   │
└────────────────────────────────────────────────────────────────────┘
```
...
- **NAT Gateway** = managed, in public subnet, allows private instances to reach internet (outbound only)
- **Security Groups** = STATEFUL (allow inbound → return traffic auto-allowed), default DENY all inbound
- **NACLs** = STATELESS (must allow BOTH inbound and outbound), default ALLOW all
- **Security Groups can reference other SGs** — this is how you isolate tiers (web → app → db)
- **VPC Peering** = connect 2 VPCs (no transitive routing!)
- **Transit Gateway** = central hub connecting many VPCs + on-prem (supports transitive routing)

---

## Concept 5: IAM and Security (30% of Exam)

### IAM Components
```
AWS Account
  ├── Users     → People (username + password/access keys)
  ...
  └── Policies  → JSON documents defining permissions
                  Managed (AWS or custom) or Inline (embedded)
```
...
4. SCPs set the MAXIMUM permissions (ceiling) for member accounts

                  SCP (Organization ceiling)
                         ∩
         IAM Policy (User/Role permissions)
                         ∩
           Resource Policy (S3 bucket, etc.)
                         =
...
| What | Service | How |
|------|---------|-----|
| Encrypt S3 objects | SSE-S3, SSE-KMS, SSE-C | Server-side. SSE-KMS gives audit trail in CloudTrail |
| Encrypt EBS volumes | KMS | Enable "encryption by default" per Region |
| Encrypt RDS | KMS | Must enable at creation; cannot encrypt existing DB (create encrypted snapshot + restore) |
| Encrypt in transit | TLS/SSL | Use HTTPS, SSL connections to DB |
| Manage keys | KMS | AWS managed keys (free) vs Customer managed keys (you control rotation, policies) |
| Store secrets | Secrets Manager | Auto-rotates passwords. Parameter Store = simpler, free tier |
| Rotate DB password | Secrets Manager | Built-in Lambda rotation for RDS |

### Key Exam Security Services
| Service | What It Does | Remember |
|---------|-------------|----------|
| **IAM** | Users, roles, permissions | Least privilege. Roles > access keys |
| **KMS** | Encryption key management | Customer managed key = YOU control rotation |
| **Secrets Manager** | Store + auto-rotate secrets | $0.40/secret/month. Has built-in RDS rotation |
| **Parameter Store** | Store config and secrets | Free tier. No auto-rotation (unless custom Lambda) |
| **GuardDuty** | Threat detection | Analyzes CloudTrail, Flow Logs, DNS. One-click enable |
| **Inspector** | Vulnerability scanning | Scans EC2 + ECR containers for CVEs |
| **Macie** | S3 data classification | Finds PII, credit cards, SSN in S3 |
| **Security Hub** | Central security dashboard | Aggregates GuardDuty + Inspector + Config + Macie |
| **WAF** | Web firewall (Layer 7) | Attaches to ALB, CloudFront, API Gateway. NOT NLB |
| **Shield** | DDoS protection | Standard = free. Advanced = $3K/month + response team |
| **Network Firewall** | VPC-level firewall | Deep packet inspection, IPS/IDS, domain filtering |
| **Firewall Manager** | Central firewall management | Manage WAF + Shield + SG across Organization |
| **Config** | Resource compliance | Tracks changes. "required-tags" rule is common |
| **CloudTrail** | API call logging | WHO did WHAT and WHEN. Enable in all Regions |
| **Organizations + SCPs** | Multi-account governance | SCPs = permission ceiling. Don't affect management account |

---

## Concept 6: High Availability vs Fault Tolerance

```
High Availability (HA)         │  Fault Tolerance (FT)
───────────────────────────────┼──────────────────────────────
Minimize downtime              │  ZERO downtime
Quick recovery from failure    │  No impact from failure
Example: Multi-AZ RDS          │  Example: Active-Active multi-Region
(60-120 sec failover)          │  (instant failover, no data loss)
Cheaper                        │  Expensive
```
...
|---|----------|-------------|
| **Protects against** | AZ failure (hardware, power) | Entire Region failure |
| **RDS** | Synchronous standby, auto failover | Cross-Region read replica (async, manual promote) |
| **Aurora** | 6 copies across 3 AZs automatic | Aurora Global Database (< 1s lag, < 1 min failover) |
| **S3** | Automatic (Standard stores across 3+ AZs) | Cross-Region Replication (CRR) |
| **DynamoDB** | Automatic | Global Tables (active-active, multi-Region) |
| **Cost** | Low additional cost | Significantly more expensive |
| **When to choose** | Default. Always use Multi-AZ | Only when exam says "Region failure" or "global users" |

---

## Concept 7: Disaster Recovery Strategies (Heavily Tested)
...
┌──────────────┬──────────┬─────────────┬─────────────────────────────┐
│  Strategy    │   RPO    │    RTO      │  What It Means              │
├──────────────┼──────────┼─────────────┼─────────────────────────────┤
│ Backup &     │  Hours   │ 24+ hours   │ Take backups. Restore when  │
│ Restore      │          │             │ disaster happens. Cheapest. │
├──────────────┼──────────┼─────────────┼─────────────────────────────┤
│ Pilot Light  │ Minutes  │ Tens of     │ Core systems always ON      │
│              │          │ minutes     │ (DB replicating). Scale up  │
│              │          │             │ app/web servers on failover.│
├──────────────┼──────────┼─────────────┼─────────────────────────────┤
│ Warm Standby │ Seconds  │ Minutes     │ Scaled-down FULL copy       │
│              │          │             │ running. Scale up during    │
│              │          │             │ failover.                   │
├──────────────┼──────────┼─────────────┼─────────────────────────────┤
│ Multi-Site   │ Near-    │ Near-       │ Full production in 2+       │
│ Active/Active│ zero     │ zero        │ Regions. Most expensive.    │
└──────────────┴──────────┴─────────────┴─────────────────────────────┘
```

**Exam rule:** Pick the **cheapest strategy that meets the stated RTO/RPO**. If the question says "RTO of 1 hour", Warm Standby or even Pilot Light works — don't choose Active/Active (too expensive).

**RPO** = How much DATA can you afford to lose? (time since last backup)
**RTO** = How much DOWNTIME can you afford? (time to recover)

---

## Concept 8: Load Balancers (Know the Differences)

| | ALB (Application) | NLB (Network) | GLB (Gateway) |
|---|-------------------|---------------|----------------|
| **Layer** | Layer 7 (HTTP/HTTPS) | Layer 4 (TCP/UDP/TLS) | Layer 3 (IP) |
| **Use case** | Web apps, microservices, containers | Ultra-low latency, gaming, IoT | Third-party firewalls, IDS |
| **Features** | Path/host routing, sticky sessions | Static IP, extreme performance | Transparent traffic inspection |
| **Targets** | EC2, IP, Lambda, containers | EC2, IP, ALB | EC2, IP |
| **WAF support** | YES | NO | NO |
| **SSL termination** | YES | YES (TLS) | NO |
| **Speed** | Fast | Ultra-fast (millions of req/s) | - |

**Exam rules:**
- **HTTP/HTTPS routing** → ALB
- **TCP/UDP, static IP, ultra-low latency** → NLB
- **"WebSocket"** → ALB (supports it natively)
- **WAF needed** → must use ALB (or CloudFront)
- ALB can do **path-based routing** (`/api` → service A, `/images` → service B)
- ALB can do **host-based routing** (`api.example.com` → one target, `web.example.com` → another)

---

## Concept 9: Serverless Architecture Pattern

```
                    ┌──────────────┐
  User Request ───► │ CloudFront   │ (optional CDN)
                    └──────┬───────┘
                           ▼
                    ┌──────────────┐
                    │ API Gateway  │ (REST API, throttling, auth)
                    └──────┬───────┘
                           ▼
                    ┌──────────────┐
        ...
        ┌──────────┐ ┌──────────┐ ┌──────────┐
        │ DynamoDB │ │    S3    │ │   SQS    │
        │(database)│ │ (files)  │ │(decouple)│
        └──────────┘ └──────────┘ └──────────┘
```

**When exam says "serverless":** Think Lambda + API Gateway + DynamoDB + S3
**When exam says "decouple":** Put SQS between services
**When exam says "fan-out":** Use SNS → multiple SQS queues
**When exam says "orchestrate":** Use Step Functions
...
| **Cold start fix** | Provisioned concurrency (keeps instances warm) |
| **Java cold start fix** | Lambda SnapStart (pre-warms JVM) |

---

## Concept 10: Data Transfer & Migration

```
Question: "How to move data to AWS?"
  │
  ├── How much data?
  │     ├── < 10 TB, good internet    → DataSync (online, fast)
  │     ├── 10 TB - 80 TB             → Snowball Edge
  │     ├── Up to 100 PB              → Snowmobile (truck!)
  │     └── Ongoing daily transfers   → DataSync or Direct Connect
  │
  ├── What type?
  │     ├── Database migration         → DMS (Database Migration Service)
  │     │     └── Different DB engine? → SCT (Schema Conversion Tool) + DMS
  │     ├── File server (NFS/SMB)     → DataSync
  │     ├── SFTP uploads from partner → AWS Transfer Family
  │     └── VMware VMs                → AWS DRS (Elastic Disaster Recovery)
  │
  └── How connected?
        ├── Over internet              → DataSync agent, S3 Transfer Acceleration
        ├── Dedicated private link     → Direct Connect (1/10/100 Gbps)
        └── VPN backup for DX          → Site-to-Site VPN
```

**Key exam rules:**
- **DataSync** = online transfer agent, validates data integrity, scheduled
- **S3 Transfer Acceleration** = speed up S3 uploads from global locations via CloudFront edge
- **Direct Connect** = dedicated private connection (NOT encrypted by default — add VPN on top for encryption)
- **DMS** = migrate databases with minimal downtime; supports ongoing replication
- **Snowball Edge** = physical device, shipped to you, offline bulk transfer

---

## Concept 11: Caching Strategies

```
Where to cache?
  │
  ├── At the Edge (closest to user)
  │     └── CloudFront → caches static + dynamic content globally
  │
  ├── At the API level
  │     └── API Gateway caching → caches API responses
  │
  ├── At the Application level
  │     ├── ElastiCache Redis   → session store, leaderboards, complex data types
  │     └── ElastiCache Memcached → simple key-value, multi-threaded
  │
  └── At the Database level
        └── DynamoDB DAX → microsecond reads, API-compatible with DynamoDB
```
...
| **Exam default** | **Choose Redis** (almost always) | Only if "multi-threaded" or "simplest cache" |

---

## Concept 12: Route 53 Routing Policies

| Policy | Use When | How It Works |
|--------|----------|--------------|
| **Simple** | Single resource | Returns one record. No health checks |
| **Weighted** | A/B testing, gradual migration | Split traffic by percentage (70/30) |
| **Latency** | Global users, lowest latency | Routes to Region with lowest latency |
| **Failover** | Active-passive DR | Primary + secondary. Auto-switches on health check fail |
| **Geolocation** | Country-specific content | Routes based on user's LOCATION (country/continent) |
| **Geoproximity** | Shift traffic between regions | Routes based on DISTANCE + bias value |
| **Multivalue** | Simple load balancing | Returns up to 8 healthy records (client picks one) |

**Exam tips:**
- **"Lowest latency"** → Latency routing
- **"Comply with regulations, country-specific"** → Geolocation
- **"Active-passive failover"** → Failover routing
- **"Gradually shift traffic"** → Weighted routing
- Route 53 is a **global** service (not Region-specific)

---

## Concept 13: Auto Scaling Concepts

### Scaling Types
| Type | How | Use When |
|------|-----|----------|
| **Target Tracking** | "Keep CPU at 50%" | Most common. Set target, AWS handles rest |
| **Step Scaling** | "Add 2 if CPU > 70%, add 4 if > 90%" | Different responses for different thresholds |
| **Scheduled** | "Scale to 10 instances every Friday at 6pm" | Known predictable traffic patterns |
| **Predictive** | ML forecasts traffic, pre-scales | Recurring patterns that ML can learn |

### Key Exam Rules
- Auto Scaling group spans **multiple AZs** for HA
...
- For **AZ failure resilience**: run **N instances per AZ** where N = minimum needed
  - Need 4 instances minimum across 2 AZs? Run 4 in EACH AZ (8 total)

---

## Concept 14: SQS vs SNS vs EventBridge vs Kinesis

| | SQS | SNS | EventBridge | Kinesis |
|---|-----|-----|-------------|---------|
| **Pattern** | Queue (pull) | Pub/Sub (push) | Event bus (push) | Stream (pull) |
| **Consumer** | One consumer per message | Many subscribers | Many targets | Multiple consumers replay |
| **Use** | Decouple, buffer | Fan-out notifications | Event routing, scheduling | Real-time streaming |
| **Ordering** | FIFO queue only | FIFO topic only | No ordering | Per-shard ordering |
| **Retention** | Up to 14 days | No retention (instant delivery) | 24 hours (replay) | Up to 365 days |

**Exam patterns:**
- **"Decouple two services"** → SQS between them
- **"Send to multiple targets"** → SNS (fan-out)
- **"SNS + SQS"** → Fan-out to multiple queues (each gets a copy)
- **"Schedule tasks" or "react to AWS API calls"** → EventBridge
- **"Real-time streaming data, millions of events"** → Kinesis Data Streams
- **"Load streaming data into S3/Redshift"** → Kinesis Data Firehose

---

## Concept 15: Hybrid Connectivity
...
On-Premises Data Center
         ...
         │     • Cheaper, good for backup
         │
         ├── AWS Direct Connect ──────────► VPC
         │     • Dedicated private connection
         │     • 1 Gbps / 10 Gbps / 100 Gbps
         │     • Takes weeks to provision
         │     • NOT encrypted (add VPN on top if needed)
         │     • Consistent latency
         │
         └── Direct Connect + VPN backup
               • DX as primary (fast, private)
               • VPN as backup (if DX fails)
               • BGP handles automatic failover
```

**Exam rules:**
- **"Consistent latency" or "dedicated connection"** → Direct Connect
- **"Quick setup" or "backup for DX"** → VPN
- **"Encrypted private connection"** → Direct Connect + VPN
- **"Redundant DX"** → Two DX connections at DIFFERENT locations (not same facility)
- **Storage Gateway** = hybrid storage (on-prem cache + S3 backend)

---

## Concept 16: Cost Optimization Decision Tree

```
Question: "What pricing model?"
  │
  ├── Steady, predictable workload (24/7 for 1-3 years)?
  │     ├── Know exact instance type?  → Reserved Instance (up to 72% off)
  │     └── Want flexibility?          → Compute Savings Plan (up to 66% off)
  │
  ├── Interruptible workload (can restart if stopped)?
  │     └── Spot Instances (up to 90% off, 2-min warning)
  │         Good for: batch, CI/CD, HPC, dev/test, stateless workers
  │
  ├── Short-term, unpredictable?
  │     └── On-Demand (pay by second, no commitment)
  │
  ├── Cannot be interrupted, need guaranteed capacity?
  │     └── On-Demand Capacity Reservation (reserve capacity in specific AZ)
  │
  └── Mixed workload?
        └── Use COMBINATION: Reserved for baseline + Spot for burst + On-Demand for rest
```

**Other cost savers:**
- **S3 Lifecycle policies** = auto-move cold data to cheaper storage
- **Right-sizing** = use Compute Optimizer to find over-provisioned instances
- **Aurora Serverless** = pay per ACU-second (no idle cost for intermittent workloads)
- **DynamoDB on-demand** = pay per request (no idle cost)
- **Lambda** = pay per invocation + duration (free tier: 1M requests/month)
- **Cost Anomaly Detection** = auto-alerts for unusual spending

---

## Concept 17: Monitoring & Audit Services

```
"WHAT happened?"      → CloudTrail  (API calls: who, what, when, from where)
"HOW is it running?"  → CloudWatch  (metrics, logs, alarms, dashboards)
"IS it configured     → AWS Config  (resource configurations, compliance rules,
  correctly?"                        change history)
"IS it secure?"       → GuardDuty   (threat detection: compromised instances,
                                      crypto mining, credential theft)
"IS data sensitive?"  → Macie       (PII/PHI discovery in S3)
"ONE dashboard?"      → Security Hub (aggregates all findings)
"Network traffic?"    → VPC Flow Logs (source/dest IP, port, allow/deny)
```

---

## Concept 18: Exam Keyword Cheat Sheet

When you see this keyword in the question, think of this answer:

| Keyword / Phrase | Think... |
|-----------------|----------|
| "minimize operational overhead" | Managed / serverless service |
| "most cost-effective" | Cheapest solution that meets ALL requirements |
| "least effort" / "least changes" | Use existing AWS features, don't re-architect |
| "decouple" | SQS between components |
| "fan-out" | SNS → multiple SQS |
| "without internet access" | VPC endpoint (gateway for S3/DynamoDB, interface for rest) |
| "compliance" / "cannot be deleted" | S3 Object Lock compliance mode |
| "encrypt at rest" | KMS (SSE-KMS for S3, EBS encryption, RDS encryption) |
| "encrypt in transit" | TLS / HTTPS / SSL certificates |
| "auto-rotate credentials" | Secrets Manager |
| "static website" | S3 + CloudFront |
| "serverless" | Lambda + API Gateway + DynamoDB |
| "real-time streaming" | Kinesis Data Streams |
| "load data into S3 from stream" | Kinesis Data Firehose |
| "query S3 data with SQL" | Athena |
| "ETL" | AWS Glue |
| "unpredictable workload" | Auto Scaling / Aurora Serverless / DynamoDB on-demand |
| "FIFO / order preserved" | SQS FIFO queue |
| "across multiple Regions" | Global: Aurora Global DB, DynamoDB Global Tables, S3 CRR |
| "single sign-on" | IAM Identity Center (formerly AWS SSO) |
| "centrally manage accounts" | AWS Organizations + SCPs |
| "prevent accidental deletion" | S3 versioning + MFA Delete, or Object Lock |
| "SFTP upload" | AWS Transfer Family |
| "Docker / containers" | ECS (simpler) or EKS (Kubernetes) |
| "no server management for containers" | Fargate |
| "HPC / high-performance file system" | FSx for Lustre |
| "Windows file share / SMB" | FSx for Windows File Server |
| "Linux shared file system / NFS" | EFS |
| "hybrid storage" | Storage Gateway |
| "DDoS protection" | Shield (Standard=free, Advanced=paid) |
| "SQL injection / XSS protection" | WAF |
| "API throttling / rate limiting" | API Gateway (usage plans) |
| "connection pooling with Lambda" | RDS Proxy |
| "data catalog" | AWS Glue Data Catalog |
| "data lake permissions" | AWS Lake Formation |
| "right-sizing" | AWS Compute Optimizer |
| "unusual spending alert" | Cost Anomaly Detection |
| "backup across services" | AWS Backup |
| "migrate database" | DMS (+ SCT if different engine) |
| "migrate servers/VMs" | AWS Elastic Disaster Recovery (DRS) |

---

## Exam Study Tips

1. **S3 Storage Classes**: Standard → IA (30d) → Intelligent-Tiering (unpredictable) → Glacier Instant (ms) → Glacier Flexible (min-hrs) → Deep Archive (12-48 hrs, cheapest)
2. **Database Selection**: RDS=relational, DynamoDB=NoSQL, Aurora=high-perf, ElastiCache=caching, Redshift=warehouse, Neptune=graph
3. **Decoupling**: SQS=buffer, SNS=fan-out, EventBridge=event routing, Step Functions=complex workflows
4. **High Availability**: Multi-AZ for DB failover, multiple AZs for Auto Scaling, Route 53 health checks, Global Accelerator for multi-region
5. **Cost Optimization**: Reserved (1-3yr steady), Spot (interruptible, 90% off), Savings Plans (flexible), On-Demand (unpredictable)
6. **Security**: VPC endpoints=private access, WAF=web firewall, Shield=DDoS, Secrets Manager=rotation, KMS=keys
7. **Migration**: Snowball (>10TB offline), DataSync (online), DMS (database), SCT (schema), Transfer Family (SFTP)
8. **Serverless**: Lambda + API Gateway + DynamoDB + S3 = fully serverless. SQS for decoupling. Aurora Serverless for variable DB
9. **Content Delivery**: CloudFront=CDN/cacheable, S3=static hosting, Global Accelerator=non-cacheable/TCP/UDP, Transfer Acceleration=S3 uploads
10. **Monitoring**: CloudTrail=API audit, CloudWatch=metrics/alarms, Config=compliance, Flow Logs=network, GuardDuty=threat detection

## 2026 Exam Key Reminders

> These are the most important things to remember for the latest SAA-C03 exam:

1. **Security is 30% of the exam** (largest domain) — every architecture starts with identity, least privilege, and private connectivity
2. **gp3 is the default EBS volume** — it decouples IOPS from storage size (unlike gp2)
3. **IMDSv2 is required** — always choose IMDSv2 over IMDSv1 to prevent SSRF attacks
4. **Disaster Recovery strategies**: Know the cost vs. RTO/RPO tradeoffs (Backup&Restore < Pilot Light < Warm Standby < Active/Active)
5. **Gateway VPC endpoints are FREE** — use for S3 and DynamoDB; interface endpoints (PrivateLink) for other services
6. **Aurora Global Database** — under 1 second replication lag, under 1 minute failover for multi-Region DR
7. **S3 Object Lock compliance mode** — even root user cannot delete; use for regulatory compliance (WORM)
8. **SCPs restrict ALL users in member accounts** — but NOT the management account
9. **Compute Savings Plans** — flexible across instance families, Regions, OS; better than standard Reserved Instances for flexibility
10. **SNS + SQS = fan-out pattern** — one message to SNS topic, delivered to multiple SQS queues simultaneously
11. **AWS Security Hub** — centralized dashboard aggregating findings from GuardDuty, Inspector, Macie, Config
12. **Cost hierarchy**: Spot (cheapest, interruptible) < Reserved/Savings Plans (committed) < On-Demand (flexible) < On-Demand Capacity Reservation (guaranteed)

---

*Generated from AWS SAA-C03 Solutions study material. Total: 690 questions + 18 concept guides.*
